...
Process Bayer RAW photographs on an NVIDIA GPU with a noise-conditioned denoiser, and test improvements using reproducible experiments. This optional Python package supplies a CUDA worker for RapidRAW's native MCP background jobs and a separate research CLI. The editor's existing NIND AI denoiser remains the lightweight option.
...
Use Python 3.11+ and a CUDA-enabled PyTorch build compatible with the host GPU. Python 3.13, PyTorch 2.12.0/CUDA 13.0 and an RTX 5060 Ti were used during development. Create a separate environment; do not change another application's environment.
...
The default reference sampler requires no compiler. For faster NVIDIA inference, install the authors' sampling extension in the same environment:
...
Build RapidRAW with the `mcp` feature and rebuild its MCP server as described in [MCP setup](../mcp/README.md). After installing this package and the checkpoint on the GPU host, set these variables in the environment that launches the server:

```sh
export RAPIDRAW_NONLOCAL_PYTHON=/path/to/denoise/.venv/bin/python
export RAPIDRAW_NONLOCAL_CHECKPOINT=/path/to/model-cache/nonlocal-raw-weights.pt
export RAPIDRAW_DENOISE_SAMPLER=cuda
```

The CUDA sampler setting requires the extension above. Omit it to use the slower PyTorch reference sampler on CUDA. Neither path silently falls back to CPU. The worker checks the converted checkpoint SHA-256 `c16747d852b93a95908792cdbac901f89cca98e35b91ea7db6214de42fbd3cad`. `rapidraw_models` reports configuration and checksum status separately from runtime CUDA availability, which is checked when a job starts.
...
Predictions are cached under the workspace's `nonlocal-cache`, keyed by input/checkpoint checksums and inference configuration. Changing intensity reuses the full prediction. Every cache read verifies the result manifest, dimensions and checksum. Corrupt entries fail explicitly. Completed DNG sessions are independent of this cache; verified prediction directories may be removed while no job is active to reclaim space. There is no automatic cache eviction.
...
This first integration is an MCP feature on a configured GPU host. The desktop denoise dialog continues to offer its existing methods. A Mac can open the resulting DNG; automatic desktop-to-server submission, transfer, local Metal inference and ONNX conversion are not included.
