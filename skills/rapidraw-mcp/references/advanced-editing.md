...
For fair comparisons, derive every candidate from the same undenoised parent and hold the crop, grade and review regions constant. Nonlocal is Bayer RAW-only and runs on the configured GPU host; the resulting DNG is portable to a Mac, but the job is not automatically offloaded there. Inspect the returned session at overview and native detail before accepting any method.

On a configured NVIDIA host, `start_denoise` also accepts `method: "nonlocal"` for Bayer RAW. Inspect `models.groups.nonlocal`: configuration/checkpoint verification does not guarantee CUDA availability until the worker starts. Use `quality: "balanced"` (one pass) first; `"maximum"` averages four rotations. Intensity defaults to 100 and blends in sensor space; cached full-strength predictions can be reused for other intensities. The result is a float Bayer DNG using RapidRAW's native demosaic/colour pipeline and captured edits in a new session. Open its DNG independently for normal RAW development, or continue the returned session to retain its exact edits. This is a background MCP option, not the desktop denoise dialog or automatic Mac-to-server offload. See [GPU setup](../../../denoise/README.md#native-raw-denoise-through-mcp). Do not route Nonlocal through the synchronous `denoise` compatibility tool.
