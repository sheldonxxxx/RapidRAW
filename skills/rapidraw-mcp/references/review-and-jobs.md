...
1. Inspect `models` for AI; install missing assets only when needed and authorized. Call `start_denoise` with parent session, latest revision, `method: "ai"` or `"bm3d"`, and intensity. Source capture and asset verification precede the job response; computation runs in a worker.
...
4. To stop, call `cancel_job`, then poll until terminal. Cancellation is cooperative between BM3D patches/AI tiles; an inference already executing may finish first. It does not kill the editing engine. If completion won the race, use the returned completed status.
