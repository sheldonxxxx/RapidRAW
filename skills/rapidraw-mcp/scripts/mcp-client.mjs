...
if (options['timeout-ms'] !== undefined && (!Number.isSafeInteger(Number(options['timeout-ms'])) || Number(options['timeout-ms']) < 1)) {
...
const { StdioClientTransport } = await import(pathToFileURL(requireFromServer.resolve('@modelcontextprotocol/client/stdio')).href);
...
let sequence = 0;
const summaryKeys = ['session_id', 'revision', 'mask_id', 'dimensions', 'working_path', 'source_path',
  'source_unchanged', 'path', 'width', 'height', 'rendered_width', 'rendered_height', 'region',
  'coordinates', 'mask_coverage', 'format', 'bit_depth', 'bytes', 'verified', 'warnings',
  'count', 'sessions', 'protocol_version', 'methods', 'workspace', 'error', 'code', 'message'];
...
try {
  ...
  console.log(JSON.stringify({ ready: true, output_dir: outputDir, note: 'MCP connected; call capabilities to check native engine startup.' }));
  ...
  for await (const line of input) {
    ...
    try {
      ...
      if (request.file) {
        ...
        if (request.index !== undefined && (!Array.isArray(saved) || !Number.isInteger(request.index) || request.index < 0 || request.index >= saved.length)) throw new Error('index must select an existing request in the file array');
        const selected = request.index === undefined ? saved : saved[request.index];
        if (!selected || typeof selected !== 'object' || Array.isArray(selected)) throw new Error('File must contain a request object, or supply index for an array');
        request = { ...selected, arguments: { ...selected.arguments, ...request.arguments } };
      }
      if (request.close === true) break;
      if (request.pick !== undefined && (!Array.isArray(request.pick) || !request.pick.every(p => typeof p === 'string'))) throw new Error('pick must be an array of field paths');
      const kinds = [typeof request.tool === 'string', typeof request.resource === 'string', Array.isArray(request.list_tools)];
      ...
      if (!Number.isInteger(timeout) || timeout < 100 || timeout > 1800000) throw new Error('timeout_ms must be 100..1800000');
    } catch (error) {
    ...
    try {
      let result;
      if (request.tool) result = await client.callTool({ name: request.tool.startsWith('rapidraw_') ? request.tool : `rapidraw_${request.tool}`, arguments: request.arguments ?? {} }, { timeout: request.timeout_ms ?? 300000 });
      else if (request.resource) result = await client.readResource({ uri: request.resource });
      else {
        const listing = await client.listTools();
        result = { structuredContent: { tools: listing.tools.filter(t => request.list_tools.length === 0 || request.list_tools.includes(t.name) || request.list_tools.includes(t.name.replace(/^rapidraw_/, ''))) } };
      }
      ...
      const data = result.structuredContent ?? result.contents ?? (result.content ?? []).filter(b => b.type !== 'image');
      const envelope = { operation: request.tool ?? request.resource ?? 'list_tools', isError: !!result.isError, elapsed_ms: Date.now() - started, data: redact(data), images };
      const responsePath = `${prefix}.json`;
      await writeFile(responsePath, JSON.stringify(envelope, null, 2));
      const brief = request.pick ? select(envelope.data, request.pick) : (result.isError || request.list_tools ? envelope.data : Object.fromEntries(summaryKeys.filter(k => envelope.data?.[k] !== undefined).map(k => [k, envelope.data[k]])));
      ...
      console.log(JSON.stringify({ isError: true, phase: 'transport-or-output', message: String(error), elapsed_ms: Date.now() - started, next: 'Reconnect and inspect persisted state before retrying; the operation may have completed.' }));
      process.exitCode = 1;
      break;
    }
  }
