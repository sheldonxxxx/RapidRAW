...
const report = {
  schema_version: 1,
  scope: 'Native enhancement including render, model and derived state; export measured separately',
...
try {
  ...
  for (const model of manifest.models) {
    await call('install_enhancement_model', {
      ...
      sha256: model.sha256 ?? (await hash(resolve(model.path))),
    });
  }
    ...
    for (const profile of profiles) {
      ...
      for (let iteration = 0; iteration < (fixture.runs ?? 2); iteration += 1) {
        ...
        const entry = {
          ...
          request: { ...fixture.request, profile, provider: process.env.RAPIDRAW_ENHANCEMENT_PROVIDER ?? 'cpu' },
        };
          ...
          const created = await call('set_adjustments', {
            ...
            patch: {
              masks: [
                {
                  id: maskId,
                  name: 'Benchmark source selection',
                  visible: true,
                  invert: false,
                  opacity: 100,
                  adjustments: {},
        ...
        try {
          ...
          entry.export = await call('export', {
          ...
          const rendered = await call('render', {
            session_id: result.session_id,
            format: 'png',
            long_edge: reviewLongEdge,
            ...(result.mask_id ? { mask_id: result.mask_id, mask_mode: 'grayscale' } : {}),
          });
          ...
          if (
            ...
            !(entry.error.includes(fixture.expected_error) || entry.error_code === fixture.expected_error)
          ) {
            entry.status = 'error';
        ...
        if (result && result.session_id !== parent.session_id)
          await call('close_session', { session_id: result.session_id });
        await call('close_session', { session_id: parent.session_id });
        report.cases.push(entry);
        await record();
        process.stdout.write(
          `${id}: ${entry.status}, enhance ${Math.round(entry.enhance_ms)}ms, export ${Math.round(entry.export_ms ?? 0)}ms\n`,
        );
      }
    }
  }
