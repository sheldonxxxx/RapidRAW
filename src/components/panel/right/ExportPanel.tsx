...
interface ExportPanelProps {
  ...
  setExportState: (value: Partial<ExportState> | ((current: ExportState) => Partial<ExportState>)) => void;
...
export default function ExportPanel({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!selectedFormat) throw new Error(`Unsupported export format: ${fileFormat}`);
      ...
      setExportState({
        errorMessage: typeof _error === 'string' ? _error : t('export.status.failed'),
        progress,
        status: Status.Error,
      });
    }
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      console.error('Failed to cancel:', _error);
      setExportState((current: ExportState) =>
