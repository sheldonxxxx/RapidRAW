...
async function run(command, args, cwd = root, env = {}) {
  ...
  const entry = { command, args, cwd, started_at: new Date().toISOString() };
  records.push(entry);
  await new Promise((resolve, reject) => {
    ...
    child.once('exit', (code, signal) => {
      ...
      entry.signal = signal;
      entry.elapsed_ms = Date.now() - start;
...
try {
  ...
  if (args.has('--build')) {
    ...
    if (process.platform === 'win32')
      assert.ok(npmArgs, 'On Windows launch through npm run test:setup so npm_execpath is available');
    const npm = async (...args) => run(npmCommand, [...(npmArgs ?? []), ...args]);
    await npm('ci');
    await npm('ci', '--prefix', 'mcp');
    await npm('run', 'build');
    await npm('test', '--prefix', 'mcp');
  ...
  if (args.has('--native')) {
    ...
    for (const script of [
      ...
      'portable-sessions-e2e.mjs',
      'operation-jobs-e2e.mjs',
      'export-interoperability-e2e.mjs',
    ])
      await run(process.execPath, [join(root, 'mcp', 'scripts', script)], root, {
        RAPIDRAW_WORKSPACE: join(output, script.replace('.mjs', '')),
      });
  }
} catch (error) {
  failure = error;
  ...
  await writeFile(
    join(output, 'setup-summary.json'),
    JSON.stringify(
      {
        ...
        fresh_machine: process.env.RAPIDRAW_FRESH_MACHINE === '1',
        fresh_machine_declaration:
          process.env.RAPIDRAW_FRESH_MACHINE === '1'
            ? 'Declared by runner; requires an actually fresh host/VM and empty dependency/model caches'
            : 'Existing host; this run cannot establish fresh-machine installation',
        built: args.has('--build'),
        native_requested: args.has('--native'),
        records,
      },
      null,
      2,
