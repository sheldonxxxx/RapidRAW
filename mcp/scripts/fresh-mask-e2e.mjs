...
} from './fresh-mask-contract.mjs';

assert.ok(
  process.env.RAPIDRAW_PHOTO_MANIFEST,
  'Set RAPIDRAW_PHOTO_MANIFEST to a manifest with pre-inference native include/exclude/edge annotations',
);
...
async function snapshot(group, session_id, mask_id, directory, phase, baselineReference) {
  ...
  async function triplet(label, geometry) {
    const response = await h.call(
      'render',
      { session_id, mask_id, mask_mode: 'overlay', format: 'png', ...geometry },
    ...
    for (const [i, block] of response.images.entries()) {
      ...
      if (geometry.region)
        assert.deepEqual(
          ...
          'Native regions must never be resized',
        );
      if (dimensions) assert.deepEqual([decoded.width, decoded.height], dimensions);
      else dimensions = [decoded.width, decoded.height];
      artifacts.push({
        path,
    ...
    artifacts.push({
      path: metadataPath,
      sha256: await hashFile(metadataPath),
  ...
  for (const edge of [1024, 768, 512, 256]) {
    overview = await triplet('overview', { long_edge: edge });
  ...
  const statePath = join(directory, 'editable-state.json');
  await exclusive(statePath, state);
  ...
  const record = unreviewedMaskRecord(group, metrics, {
    first_attempt: phase === 'baseline',
    partition: phase === 'baseline' ? 'fresh-holdout' : 'regression',
    phase,
    session_id,
    mask_id,
    ...
    coordinate_space: 'full native mask canvas before crop; no user geometry or grade applied',
    source_processing:
      'Full-resolution original used for native inference; only explicit overview requests resize output.',
  });
  const reviewPath = join(directory, 'review-required.json');
  await exclusive(reviewPath, record);
  return {
    group,
    directory,
    session_id,
    mask_id,
    revision: state.revision,
    state_digest: digest(state.adjustments),
    review_path: reviewPath,
    review_sha256: await hashFile(reviewPath),
    record,
  };
}
async function quantitativeCheck(result) {
  try {
    await h.check(
      `${result.record.phase}_annotated_coverage_${result.group.id}`,
      ['tool:mask_generate'],
      async () => {
    ...
    failures.push({
      fixture: result.group.id,
      phase: result.record.phase,
      failure_kind: 'annotated_coverage',
      error: String(error),
    });
  }
}
...
try {
  ...
  for (const group of manifest.groups) {
    const captured = await h.fixture(group.sources[0].path, group.id, {
  ...
  for (const group of manifest.groups) {
    ...
    if (unavailable) {
      skipped.push({ fixture: group.id, phase: 'baseline', reason: unavailable });
      await h.skip(`baseline_${group.id}`, ['tool:mask_generate'], unavailable);
      continue;
    }
    try {
      ...
      assert.equal(
        ...
        'Opened original must match the pre-inference annotated source',
      );
      ...
      await h.check(
        `baseline_artifacts_${group.id}`,
        ['tool:mask_generate', 'tool:render', 'tool:save_session'],
        async () => ({
          session_id: result.session_id,
          mask_id: result.mask_id,
          review_path: result.review_path,
          photographic_quality: 'not_reviewed',
          native_regions: group.annotations.map((a) => a.region),
        }),
      );
      await quantitativeCheck(result);
    } catch (error) {
      await failCase(group, directory, 'baseline', error);
    }
  }
  ...
  for (const baseline of completed.filter((result) => result.group.refinement)) {
    ...
    const directory = join(workspace, 'corrections', group.id, 'point-refinement');
    await mkdir(directory, { recursive: true });
    await exclusive(join(directory, 'fixture-manifest.json'), {
      ...group,
      partition: 'regression',
      first_attempt: false,
    });
    if (unavailable) {
      skipped.push({ fixture: group.id, phase: 'correction', reason: unavailable });
      await h.skip(`correction_${group.id}`, ['tool:mask_generate'], unavailable);
      continue;
    }
    try {
      const fork = await mutation('fork_session', {
        session_id: baseline.session_id,
        expected_revision: baseline.revision,
        label: 'Independent point correction',
      });
      const refined = await mutation(
        'mask_generate',
        {
          session_id: fork.session_id,
          expected_revision: fork.revision,
          kind: 'subject',
          refine: { mask_id: baseline.mask_id },
          ...group.refinement,
        },
        ['session_id', 'mask_id'],
      );
      const result = await snapshot(group, fork.session_id, refined.mask_id, directory, 'correction', {
        session_id: baseline.session_id,
        mask_id: baseline.mask_id,
        path: baseline.review_path,
        sha256: baseline.review_sha256,
      });
      corrections.push(result);
      await h.check(
        `correction_preserves_baseline_${group.id}`,
        ['tool:fork_session', 'tool:mask_generate'],
        async () => {
          const unchanged = await readEditableMaskSession(h, baseline.session_id);
          assert.equal(unchanged.revision, baseline.revision);
          assert.equal(digest(unchanged.adjustments), baseline.state_digest);
          ...
          return {
            baseline_session_unchanged: true,
            first_attempt_record_unchanged: true,
            corrected_session_id: result.session_id,
            partition: 'regression',
          };
        },
      );
      ...
      await failCase(group, directory, 'correction', error);
    }
  }
  const entry = (result) => ({
    fixture: result.group.id,
    review_path: result.review_path,
    quantitative_status: result.record.quantitative_status,
  });
  await exclusive(join(workspace, 'mask-review-index.json'), {
    status: 'not_reviewed',
    photographic_quality: 'not_reviewed',
    first_attempts: completed.map(entry),
    corrections: corrections.map(entry),
    failures,
    skipped,
    note: 'Review each preserved first attempt separately; corrections cannot replace baseline judgments.',
  });
} catch (error) {
  failures.push({
    phase: 'setup_or_recording',
    failure_kind: 'execution',
    error: String(error),
    photographic_quality: 'not_reviewed',
  });
