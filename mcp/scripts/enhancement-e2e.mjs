...
assert.ok(
  process.env.RAPIDRAW_ENHANCEMENT_MANIFEST,
  'Set RAPIDRAW_ENHANCEMENT_MANIFEST to a private fixture/model manifest',
);
...
const render = async (session_id, path, mask_id) => {
  const response = await call('render', {
    ...
    format: 'png',
    long_edge: 1600,
    ...(mask_id ? { mask_id, mask_mode: 'grayscale' } : {}),
  });
...
try {
  ...
  assert.ok(capabilities.methods.includes('enhance'));
  for (const item of manifest.models) {
    await call('install_enhancement_model', {
      model_id: item.id,
      path: resolve(item.path),
      sha256: item.sha256 ?? (await hash(resolve(item.path))),
    });
  }
  for (const fixture of manifest.cases) {
    assert.match(fixture.id, /^[a-zA-Z0-9_-]+$/);
    const directory = join(workspace, fixture.id);
    await mkdir(directory);
    const source = resolve(fixture.path),
    ...
    if (fixture.mask_path) {
              ...
              subMasks: [
                {
                  ...
                  parameters: {
                    startX: 0,
                    startY: 0,
                    endX: parent.dimensions.width,
                    endY: parent.dimensions.height,
                    maskDataBase64: `data:image/png;base64,${bitmap.toString('base64')}`,
                    rotation: 0,
                    orientationSteps: 0,
      ...
      mask_id = parent.adjustments.masks.at(-1).id;
    ...
    const args = {
      ...
      request: {
        ...fixture.request,
        provider: process.env.RAPIDRAW_ENHANCEMENT_PROVIDER ?? fixture.request.provider ?? 'auto',
      },
      ...(mask_id ? { mask_id } : {}),
    };
    await assert.rejects(
      call('enhance', { ...args, expected_revision: parent.revision + 1 }),
      (error) => error.code === 'REVISION_CONFLICT',
    );
    if (fixture.expected_error) {
      await assert.rejects(
        call('enhance', args),
        (error) => error.code === fixture.expected_error || String(error).includes(fixture.expected_error),
      );
    ...
    const result = await call('enhance', args);
    ...
    if (fixture.request.operation === 'deblur' || fixture.request.operation === 'upscale') {
      assert.notEqual(result.session_id, parent.session_id);
      assert.equal(result.edits_baked, true);
      assert.equal(enhanced.is_raw, false);
      ...
      const [w, h] = fixture.request.region?.slice(2) ?? [parent.dimensions.width, parent.dimensions.height];
      assert.deepEqual(enhanced.dimensions, { width: w * scale, height: h * scale });
    } else {
      assert.equal(result.session_id, parent.session_id);
      if (mask_id) {
        assert.equal(result.mask_id, mask_id);
        assert.deepEqual(enhanced.adjustments.masks[0].adjustments, parent.adjustments.masks[0].adjustments);
      }
      await call('undo', { session_id: result.session_id });
      assert.deepEqual(
        (await call('get_session', { session_id: result.session_id, include_adjustments: true })).adjustments,
        parent.adjustments,
      );
    ...
    if (fixture.background_check) {
      ...
      const cancelled = await jobs.dispatch('cancel_operation_job', { job_id: job.job_id });
      ...
      do {
        ...
        finished = await jobs.dispatch('get_operation_job', { job_id: job.job_id });
    ...
    await jobs.close();
    await bridge.close();
    bridge = new NativeBridge(options);
    jobs = new OperationJobs(bridge, options);
    assert.equal(
      await render(result.session_id, join(directory, 'restart.png')),
      photo,
      'Rendered result changed after restart',
    );
    const entry = {
      id: fixture.id,
      ...
      result,
      export: exported,
      baseline: join(directory, 'baseline.png'),
      preview: join(directory, 'result.png'),
      quality: 'not_reviewed',
    };
    await writeFile(join(directory, 'result.json'), JSON.stringify(entry, null, 2));
  ...
  report.checks.push(
    'source preservation',
    'revision rejection',
    'mask history or independent restoration session',
    'restart pixel equality',
    '16-bit sRGB export',
  );
} catch (error) {
  ...
  await jobs.close();
  await bridge.close();
  await writeFile(join(workspace, 'enhancement-report.json'), JSON.stringify(report, null, 2));
}
