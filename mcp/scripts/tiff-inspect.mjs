...
export function inspectTiff16(bytes) {
  ...
  for (let index = 0; index < u16(ifd); index += 1) {
    ...
    tags.set(
      tag,
      Array.from({ length: count }, (_, i) => (type === 3 ? u16(offset + i * size) : u32(offset + i * size))),
    );
  }
  const width = tags.get(256)?.[0],
    height = tags.get(257)?.[0];
  const bits = tags.get(258),
    channels = tags.get(277)?.[0] ?? 1;
  assert.ok(width > 0 && height > 0);
  assert.ok(
    bits?.every((n) => n === 16),
    `Expected true 16-bit samples; got ${bits}`,
  );
  ...
  assert.ok(
    sampleFormat.every((n) => n === 1),
    'Expected unsigned integer samples',
  );
  ...
  return {
    ...
    bit_depth: 16,
    compression,
    unique_values: distinct.size,
    samples,
    non_8bit_replicated_samples: nonReplicatedSamples,
  };
}
