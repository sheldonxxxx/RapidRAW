...
const binary = process.env.RAPIDRAW_BINARY,
  source = process.env.RAPIDRAW_TEST_IMAGE;
...
const sourceSidecar = await readFile(`${source}.rrdata`).catch((error) => {
  if (error.code === 'ENOENT') return null;
  throw error;
});
let client, coverage, failure;
async function verified(name, requirements, details, level = 'native_assertion') {
...
async function connect() {
  if (coverageEnabled) {
    coverage = await createNativeHarness({
      suite: 'review-jobs-regression',
      workspace,
      binary,
      env: { ORT_DYLIB_PATH: process.env.ORT_DYLIB_PATH ?? resolve(binary, '../resources/libonnxruntime.dylib') },
    });
    client = coverage.client;
    await coverage.fixture(source, 'photographic-review-denoise-input');
  ...
  await client.connect(
    new StdioClientTransport({
      ...
      env: {
        ...process.env,
        ORT_DYLIB_PATH: process.env.ORT_DYLIB_PATH ?? resolve(binary, '../resources/libonnxruntime.dylib'),
      },
    }),
  );
}
async function call(method, args = {}, error = false) {
  ...
  const result = coverage
    ? (await coverage.call(method, args, { expectError: error })).result
...
async function waitJob(id, expected = 'succeeded') {
  ...
  while (Date.now() < deadline) {
    ...
    assert.ok(job.progress_percent >= last, 'progress cannot regress within an attempt');
    last = job.progress_percent;
...
try {
  ...
  for (const m of [
    ...
    assert.ok(caps.methods.includes(m));
  ...
  const mask = await data('mask_create', {
    session_id: sid,
    type: 'linear',
    name: 'Sky transition',
    parameters: { startX: 0, startY: height / 2, endX: width, endY: height / 2, range: 80 },
    adjustments: { exposure: -0.5 },
  });
  const before = await data('get_session', { session_id: sid, include_adjustments: true });
  const version = await data('save_version', {
    session_id: sid,
    label: 'Accepted reference',
    expected_revision: before.revision,
  });
  await data('save_session', { session_id: sid });
  const manifest = join(workspace, 'sessions', sid, 'session.json'),
    sidecar = `${before.working_path}.rrdata`;
  const unchanged = [await hash(manifest), await hash(sidecar), await hash(before.working_path)];
  const variants = [
    { label: 'Reference', version_id: version.version_id },
    { label: 'Cooler', patch: { temperature: -15 } },
    { label: 'Warmer', patch: { temperature: 15 } },
    { label: 'Mask off', disabled_masks: [mask.mask_id] },
  ];
  const comparison = await call('render_compare', {
    session_id: sid,
    variants,
    region: { x: 0, y: 0, width, height },
    long_edge: 512,
  });
  assert.equal(comparison.images.length, 4);
  assert.equal(comparison.data.saved_revision, before.revision);
  for (let i = 0; i < 4; i++) {
  ...
  for (const alternative of comparedPixels.slice(1))
    assert.ok(
      pixelDifference(comparedPixels[0], alternative).maximum > 0,
      'Decoded comparison alternatives must change pixels',
    );
  await verified(
    'temporary_comparisons_change_decoded_pixels',
    ['tool:render_compare'],
    { variants: 4, saved_revision: before.revision },
    'pixel_assertion',
  );
  const diagnostic = await call('inspect_adjustments', { session_id: sid, long_edge: 512 });
  assert.equal(diagnostic.images.length, 4);
  assert.ok(diagnostic.data.exposure_map.min_ev < 0);
  for (let i = 0; i < 4; i++)
    await writeFile(join(workspace, `inspection-${i}.png`), Buffer.from(diagnostic.images[i].data, 'base64'));
  ...
  await verified(
    'diagnostic_maps_leave_manifest_sidecar_source_and_state_unchanged',
    ['tool:inspect_adjustments', 'tool:render_compare'],
    { min_ev: diagnostic.data.exposure_map.min_ev, immutable_hashes: unchanged },
  );
  await data(
    'render_compare',
    { session_id: sid, variants: [{ label: 'same' }, { label: 'warped', patch: { transformVertical: 10 } }] },
    true,
  );
  await data(
    'render_compare',
    { session_id: sid, variants: [{ label: 'same' }, { label: 'missing', disabled_masks: ['missing'] }] },
    true,
  );
  const sub = before.adjustments.masks[0].subMasks[0];
  await data('mask_update', {
    session_id: sid,
    mask_id: mask.mask_id,
    patch: {
      subMasks: [
        { ...sub, parameters: { ...sub.parameters, fadeBefore: 40, fadeAfter: 120, falloff: 'smootherstep' } },
      ],
    },
  });
  ...
  for (let i = 0; i < 34; i++)
    await data('set_adjustments', { session_id: sid, patch: { exposure: i % 2 ? 0.1 : 0.2 } });
  assert.equal((await data('history', { session_id: sid })).entries.length, 32);
  assert.ok(
    (await data('list_versions', { session_id: sid })).versions.some((v) => v.version_id === version.version_id),
  );
  const edited = await data('get_session', { session_id: sid });
  const restored = await data('restore_version', {
    session_id: sid,
  ...
  assert.deepEqual(
    (await data('get_session', { session_id: sid, include_adjustments: true })).adjustments,
    before.adjustments,
  );
  await data(
    'restore_version',
    { session_id: sid, version_id: version.version_id, expected_revision: edited.revision },
    true,
  );
  await verified(
    'named_reference_survives_history_truncation_and_revision_conflict',
    ['tool:save_version', 'tool:list_versions', 'tool:restore_version', 'tool:history'],
    { history_limit: 32, restored_revision: restored.revision },
  );
  ...
  await verified(
    'bm3d_cancellation_keeps_parent_editing_and_rendering_available',
    ['tool:start_denoise', 'tool:cancel_job', 'tool:get_job'],
    { cancelled_job: cancelling.job_id },
  );
  ...
  assert.deepEqual(
    ...
    'Job must inherit captured edits, not later parent changes',
  );
  assert.equal(derived.is_raw, captured.is_raw);
  assert.equal(derived.metadata.derivedFrom.revision, captured.revision);
  await verified('completed_job_inherits_captured_state_and_raw_domain', ['tool:start_denoise', 'tool:get_job'], {
    source_revision: captured.revision,
    result_session_id: derived.session_id,
    is_raw: derived.is_raw,
  });
  ...
  assert.ok(
    !(await data('list_sessions')).sessions.some((s) => s.session_id === derived.session_id),
    'Job polling must not reopen explicitly closed sessions',
  );
  ...
  assert.deepEqual(
    (await data('get_session', { session_id: derived.session_id, include_adjustments: true })).adjustments,
    captured.adjustments,
  );
  assert.ok(
    (await data('list_versions', { session_id: sid })).versions.some((v) => v.version_id === version.version_id),
  ...
  assert.equal(resumed.job_id, interrupted.job_id);
  await data('cancel_job', { job_id: resumed.job_id });
  await waitJob(resumed.job_id, 'cancelled');
  await verified(
    'restart_recovery_preserves_completed_results_and_explicit_resume_identity',
    ['tool:resume_job', 'tool:get_job', 'tool:list_versions', 'tool:close_session'],
    { resumed_job: resumed.job_id, attempt: resumed.attempt, completed_result: derived.session_id },
  );
  if (process.env.RAPIDRAW_TEST_AI === '1') {
    ...
    assert.ok(
      models.groups.denoise.assets.every((a) => a.verified || a.installed_app_copy_available),
      'AI test requires already installed assets; it never downloads',
    );
    ...
    await verified(
      'ai_background_completion_and_inference_cancellation',
      ['tool:start_denoise', 'tool:cancel_job', 'parameter:start_denoise.method="ai"'],
      { completed_job: ai.job_id, cancelled_job: stopping.job_id },
    );
  } else if (coverage)
    await coverage.skip(
      'ai_background_completion_and_inference_cancellation',
      ['parameter:start_denoise.method="ai"'],
      'Set RAPIDRAW_TEST_AI=1 with installed local denoise assets.',
    );
  ...
  await verified(
    'completed_job_is_discovered_after_restart_without_prior_polling',
    ['tool:start_denoise', 'tool:get_job'],
    { job_id: unpolled.job_id },
  );
  assert.equal(await hash(source), sourceHash);
  await writeFile(
    join(workspace, coverageEnabled ? 'review-jobs-summary.json' : 'summary.json'),
    JSON.stringify(
      {
        ok: true,
        workspace,
        source_sha256: sourceHash,
        tools: caps.methods.length,
        checks: [
          'temporary native comparisons',
          'combined exposure and actual differences',
          'named references survive history truncation and restart',
          'asymmetric smooth gradient rendering',
          'editing and rendering during denoise',
          'cooperative cancellation without engine restart',
          'captured edits inherited by derived result',
          'durable completed result',
          'interruption detection and explicit resume',
          'completion without polling',
          'source hash unchanged',
        ],
        ai_tested: process.env.RAPIDRAW_TEST_AI === '1',
      },
      null,
      2,
    ),
