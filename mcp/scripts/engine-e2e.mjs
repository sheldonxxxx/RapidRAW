...
async function call(method, args = {}, expectError = false) {
  ...
  const result = coverage ? (await coverage.call(method, args, { expectError })).result : await client.callTool({ name: `rapidraw_${method}`, arguments: args }, { timeout: 300000 });
...
try {
  ...
  assert.deepEqual(new Set(discoveredTools.map((tool) => tool.name.replace(/^rapidraw_/, ''))), new Set(capabilities.methods));
  ...
  for (const [index, source] of sources.entries()) {
    ...
    await evidenceCheck(`exposure_preview_${index}`, ['tool:set_adjustments', 'tool:render', 'adjustment:exposure'], () => { assert.notEqual(edited, original, 'Real exposure edit must change rendered pixels'); return { encoded_preview_changed: true }; });
    ...
    await evidenceCheck(`metadata_roundtrip_${index}`, ['tool:set_metadata', 'tool:get_metadata'], async () => { const metadata = data(await call('get_metadata', { session_id })).metadata; assert.equal(metadata.rating, 4); assert.deepEqual(metadata.tags, ['mcp-e2e']); return { rating: metadata.rating, tags: metadata.tags }; });
    ...
    const precision = await evidenceCheck(`full_resolution_tiff_precision_${index}`, ['tool:export', 'parameter:export.format="tiff"', 'parameter:export.bit_depth=16'], async () => inspectTiff16(await readFile(masterPath)), 'pixel_assertion');
    ...
    await evidenceCheck(`session_restart_${index}`, ['tool:close_session', 'tool:list_sessions', 'tool:get_session', 'tool:render', 'tool:save_session'], () => { assert.equal(afterRestart, beforeRestart, 'Restored session must render identical pixels'); assert.deepEqual(restored.adjustments, finalState.adjustments); assert.deepEqual(restored.metadata, finalState.metadata); assert.equal(restored.revision, finalState.revision); return { identical_encoded_preview: true, restored_revision: restored.revision }; }, 'pixel_assertion');
    ...
    await evidenceCheck(`batch_outcomes_${index}`, ['tool:batch_export'], async () => {
      ...
      const success = data(await call('batch_export', { items: [{ session_id, path: successPath }], options: { format: 'jpeg', long_edge: 800 } }));
      assert.equal(success.succeeded, 1); assert.equal(success.failed, 0);
  ...
  let closingError;
  try { if (coverage) await coverage.close(failure); else await client?.close(); }
