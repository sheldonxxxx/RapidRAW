...
const root = resolve(process.env.RAPIDRAW_TEST_WORKSPACE ?? process.env.RAPIDRAW_WORKSPACE ?? `test-output/portable-live/${Date.now()}`);
...
const requirements = (method, ...fields) => [`tool:${method}`, ...fields.map((field) => `parameter:${method}.${field}`)];
...
let first, second, failure, accepted, source, reference, bundle, presetExport, lutExport, portableImage, rawBundle, rawImage;
try {
  ...
  await first.call('set_adjustments', { session_id: source.session_id, expected_revision: source.revision, patch: { exposure: 0.35, temperature: -6 } });
  await first.call('mask_create', { session_id: source.session_id, type: 'radial', name: 'Warm centre', parameters: { centerX: 96, centerY: 64, radiusX: 40, radiusY: 32, rotation: 0, feather: 0.6 }, adjustments: { exposure: 0.3 } });
  let ownedLut, ownedPreset;
  await first.check('workspace_lut_import_list_export_and_apply', requirements('manage_luts', 'action="import"', 'action="list"', 'action="export"'), async () => {
    ownedLut = (await first.call('manage_luts', { action: 'import', path: lutSource, name: 'Portable identity' })).data;
    ...
    lutExport = (await first.call('manage_luts', { action: 'export', id: ownedLut.id, path: 'portable-identity.cube' })).data;
  ...
  reference = (await first.call('save_version', { session_id: source.session_id, expected_revision: accepted.revision, label: 'Accepted portable edit' })).data;
  portableImage = await render(first, source.session_id);
  await first.check('named_fork_matches_pixels_then_edits_independently', requirements('fork_session', 'version_id', 'expected_revision', 'label'), async () => {
    await first.call('set_adjustments', { session_id: source.session_id, expected_revision: accepted.revision, patch: { exposure: -0.8 } });
    const fork = (await first.call('fork_session', { session_id: source.session_id, version_id: reference.version_id, label: 'Independent candidate' })).data;
    ...
    await first.call('set_adjustments', { session_id: fork.session_id, expected_revision: 0, patch: { exposure: 1.2 } });
    assert.equal((await session(first, source.session_id)).adjustments.exposure, -0.8);
    const invalid = await first.call('fork_session', { session_id: source.session_id, expected_revision: 0 }, { expectError: true });
  ...
  await first.check('version_diff_reports_controls_without_mutation', requirements('diff_versions', 'from_version_id'), async () => {
    const before = await session(first, source.session_id);
    const diff = (await first.call('diff_versions', { session_id: source.session_id, from_version_id: reference.version_id })).data;
    assert.ok(diff.changes.some((change) => change.path === '/adjustments/exposure' && change.before === 0.35 && change.after === -0.8));
  ...
  await first.check('selective_copy_has_individual_revision_results_and_explicit_geometry', requirements('copy_adjustments', 'geometry="exclude"', 'targets', 'mode="replace_selected"'), async () => {
    ...
    const result = (await first.call('copy_adjustments', { session_id: source.session_id, version_id: reference.version_id, keys: ['exposure', 'temperature', 'masks'], geometry: 'exclude', mode: 'replace_selected', targets: [{ session_id: target.session_id, expected_revision: 0 }, { session_id: stale.session_id, expected_revision: 0 }] }, { expectError: true })).data;
    ...
    const complete = (await first.call('copy_adjustments', { session_id: source.session_id, keys: ['temperature'], geometry: 'exclude', targets: [{ session_id: target.session_id, expected_revision: compatible.revision }] })).data;
    assert.equal(complete.succeeded, 1); assert.equal(complete.failed, 0);
  ...
  await first.check('owned_preset_saves_and_exports_dependencies', requirements('manage_presets', 'action="save"', 'action="list"', 'action="export"'), async () => {
    ownedPreset = (await first.call('manage_presets', { action: 'save', session_id: source.session_id, name: 'Portable grade', include_masks: true, include_geometry: true })).data;
    assert.ok((await first.call('manage_presets', { action: 'list' })).data.presets.some((preset) => preset.id === ownedPreset.id));
    assert.ok((await first.call('list_presets')).data.presets.some((preset) => preset.id === ownedPreset.id));
    presetExport = (await first.call('manage_presets', { action: 'export', id: ownedPreset.id, path: 'portable-preset.json' })).data;
    const exported = JSON.parse(await readFile(presetExport.path, 'utf8'));
    assert.ok(exported.lut.data_base64.length > 0); assert.ok(exported.lut.sha256.length === 64);
  ...
  await first.check('native_imported_preset_excludes_valid_retouch_patches_when_masks_disabled', requirements('manage_presets', 'action="import"'), async () => {
    ...
    await writeFile(presetPath, JSON.stringify({ id: 'native-source-id', name: 'Exclude selective content', includeMasks: false, includeCropTransform: false, adjustments: { exposure: 0.1, aiPatches: [aiPatch], crop: { x: 0, y: 0, width: 32, height: 32 } } }));
  ...
  await first.check('removing_library_entries_preserves_materialized_sessions', [...requirements('manage_luts', 'action="remove"'), ...requirements('manage_presets', 'action="remove"')], async () => {
    ...
    const outside = await first.call('manage_luts', { action: 'remove', id: 'workspace:../../outside' }, { expectError: true });
    assert.ok(outside.result.isError);
    return { rendered_after_removal: true };
  }, 'pixel_assertion');
  await first.check('bundle_export_records_complete_assets_and_no_overwrite', requirements('export_session_bundle', 'name', 'expected_revision'), async () => {
    const current = await session(first, source.session_id);
    bundle = (await first.call('export_session_bundle', { session_id: source.session_id, expected_revision: current.revision, name: 'portable-edit' })).data;
    ...
    const duplicate = await first.call('export_session_bundle', { session_id: source.session_id, name: 'portable-edit' }, { expectError: true });
    assert.match(JSON.stringify(duplicate.result), /OUTPUT_EXISTS/);
    return { manifest_sha256: bundle.manifest_sha256, assets: manifest.files.length, versions: manifest.versions.length };
  });
  if (process.env.RAPIDRAW_RAW_FIXTURE) {
    ...
    rawBundle = (await first.call('export_session_bundle', { session_id: raw.session_id, name: 'raw-portability' })).data;
  } else {
    await first.skip('photographic_raw_bundle', ['tool:export_session_bundle', 'tool:import_session_bundle'], 'Set RAPIDRAW_RAW_FIXTURE to run native photographic RAW portability in addition to TIFF RAW-domain unit tests.');
  }
  const transport = join(root, 'transport'); await mkdir(transport);
  ...
  await rename(join(root, 'source-workspace', 'sessions'), join(root, 'source-workspace', 'detached-original-sessions'));
  ...
  await second.check('restart_ignores_unpublished_import_staging_manifests', requirements('import_session_bundle'), async () => {
  ...
  await second.check('moved_bundle_import_matches_pixels_without_original_session_paths', requirements('import_session_bundle', 'path', 'expected_manifest_sha256'), async () => {
    const imported = (await second.call('import_session_bundle', { path: join(transport, 'edit'), expected_manifest_sha256: bundle.manifest_sha256 })).data;
    assert.notEqual(imported.session_id, source.session_id);
    assert.equal(imported.source_sha256, accepted.source_sha256); assert.equal(imported.is_raw, accepted.is_raw);
    ...
    assert.equal(pixelDifference(await render(second, imported.session_id), portableImage).maximum, 0);
    return { imported_session_id: imported.session_id, max_pixel_difference: 0, named_version_restored_after_bundle_deletion: true };
  }, 'pixel_assertion');
  await second.check('self_contained_preset_import_matches_render_after_library_removal', requirements('manage_presets', 'action="import"'), async () => {
    const imported = (await second.call('manage_presets', { action: 'import', path: join(transport, 'preset.json'), name: 'Relocated grade' })).data;
    ...
    const corrupt = JSON.parse(await readFile(join(transport, 'preset.json'), 'utf8')); corrupt.lut.sha256 = '0'.repeat(64);
  ...
  await second.check('corrupt_missing_traversal_and_symlink_bundle_rejection_is_atomic', requirements('import_session_bundle', 'expected_manifest_sha256'), async () => {
    ...
    for (const kind of cases) {
      const directory = join(transport, kind); await cp(exported, directory, { recursive: true });
      ...
      if (kind === 'symlink') { await rm(join(directory, manifest.session.working_path)); await symlink(fixture, join(directory, manifest.session.working_path)); }
      if (kind === 'traversal') { manifest.files[0].path = '../outside'; await writeFile(manifestPath, JSON.stringify(manifest)); }
      const result = await second.call('import_session_bundle', { path: directory, ...(kind === 'digest' ? { expected_manifest_sha256: '0'.repeat(64) } : {}) }, { expectError: true });
  ...
  if (rawBundle) await second.check('photographic_raw_domain_and_pixels_survive_workspace_move', requirements('import_session_bundle'), async () => {
    const imported = (await second.call('import_session_bundle', { path: join(transport, 'raw'), expected_manifest_sha256: rawBundle.manifest_sha256 })).data;
