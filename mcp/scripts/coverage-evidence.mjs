...
export function nativeExecutableFormat(header) {
  ...
  if (['feedface', 'feedfacf', 'cefaedfe', 'cffaedfe', 'cafebabe', 'bebafeca', 'cafebabf', 'bfbafeca'].includes(magic))
    return 'Mach-O';
...
export function sanitize(value, key = '') {
  ...
  if (typeof value === 'string' && (/base64/i.test(key) || value.length > 4096))
...
function leaves(schema, prefix, output) {
  ...
  for (const value of schema.enum ?? (schema.const === undefined ? [] : [schema.const]))
    output.add(`${prefix}=${JSON.stringify(value)}`);
...
export function requiredInventory(tools, capabilities) {
  ...
  for (const tool of tools) {
    ...
    const paths = new Set();
    leaves(tool.inputSchema, '', paths);
    for (const path of paths) required.add(`parameter:${name}.${path}`);
  }
  const paths = new Set();
  leaves(capabilities.adjustment_schema, '', paths);
...
export function observedRequirements(method, args, { tools = [], capabilities = {} } = {}) {
  ...
  function visit(value, prefix, schema = {}) {
    ...
    if (Array.isArray(value)) {
      ...
      for (const item of value) visit(item, `${prefix}[]`, itemSchema);
      return;
    }
    const variants = schema.oneOf ?? schema.anyOf ?? [];
    const discriminated = variants.find(
      (variant) => variant.properties?.type?.const !== undefined && variant.properties.type.const === value.type,
    );
    ...
    const branches = discriminated
      ? [discriminated]
      : variants.filter(
          (variant) => variant.properties?.type?.const === undefined && (!variant.type || variant.type === 'object'),
        );
  ...
  if (method === 'set_adjustments' && args.patch)
    for (const [key, value] of Object.entries(args.patch))
      visit(value, `adjustment:${key}`, adjustment?.properties?.[key]);
  const local = adjustment?.properties?.masks?.items?.properties?.adjustments;
  if (method === 'mask_create' && args.adjustments)
    for (const [key, value] of Object.entries(args.adjustments))
      visit(value, `adjustment:masks[].adjustments.${key}`, local?.properties?.[key]);
  return [...output];
}
export function aggregateEvidence(inventory, records) {
  const rows = new Map(
    inventory.map((id) => [
      ...
      {
        id,
        classification: classifyRequirement(id),
        levels: [],
        evidence: [],
        evidence_by_basis: { direct_input: [], explicit_assertion: [], derived_state: [], legacy_unspecified: [] },
        failures: [],
        skips: [],
      },
    ]),
  ...
  for (const record of records) {
    ...
    if (record.status === 'passed' && record.basis === 'derived_state') {
      if (
        record.type !== 'check' ||
        !['native_assertion', 'pixel_assertion'].includes(record.level) ||
        !Array.isArray(record.source_call_ids) ||
        !record.source_call_ids.length ||
        !record.source_call_ids.every((id) => {
          const source = byId.get(id);
          return (
            source?.evidence_format === 'rapidraw-native-evidence-v1' &&
            source.execution === 'real_native_mcp' &&
            source.type === 'call' &&
            source.status === 'passed' &&
            source.level === 'native_call' &&
            !source.expected_error
          );
        })
      )
        throw new Error(
          `Derived-state evidence ${record.id} needs an explicit assertion and successful native source calls`,
        );
    }
    for (const id of record.requirements ?? []) {
      ...
      if (record.status === 'passed' && evidenceLevels.includes(record.level)) {
        ...
        const basis =
          record.basis === 'direct_input' &&
          record.direct_input_requirements &&
          !record.direct_input_requirements.includes(id)
            ? 'explicit_assertion'
            : (record.basis ?? 'legacy_unspecified');
  ...
  return {
    schema_version: 2,
    requirements,
    groups: summarizeGroups(requirements),
    grouping_note:
      'Families group shared schema and workflow contexts for planning only. Every original requirement retains independent evidence; group membership grants no credit.',
    totals: {
      required: requirements.length,
      untested: requirements.filter((r) => !r.levels.length).length,
      ...Object.fromEntries(
        evidenceLevels.map((level) => [level, requirements.filter((r) => r.levels.includes(level)).length]),
      ),
      ...Object.fromEntries(
        ['direct_input', 'derived_state'].map((basis) => [
          basis,
          requirements.filter((r) => r.evidence_by_basis[basis].length).length,
        ]),
      ),
...
export async function createNativeHarness({
  ...
  const executable = await open(binary, 'r');
  ...
  async function runtimeSnapshot() {
    const sourcePaths = (
      await exec('git', ['ls-files', '--cached', '--others', '--exclude-standard', '-z'], { cwd: repository })
    ).stdout
      .split('\0')
      .filter(Boolean)
    ...
    const files = await Promise.all(
      [...new Set(sourcePaths)].map(async (path) => ({
        path,
        sha256: await hashFile(join(repository, path)).catch((error) => {
  ...
  const untrackedPaths = (
    await exec('git', ['ls-files', '--others', '--exclude-standard', '-z'], { cwd: repository })
  ).stdout
    .split('\0')
    .filter(Boolean)
    .sort();
  const untracked = await Promise.all(
    untrackedPaths.map(async (path) => ({ path, sha256: await hashFile(join(repository, path)) })),
  );
  ...
  const provenance = {
    ...
    source_dirty: Boolean(await git('status', '--porcelain')),
    source_diff_sha256: hashBytes(diff.stdout),
    source_tree_sha256: hashBytes(JSON.stringify({ diff: hashBytes(diff.stdout), untracked })),
    untracked_source_files: untracked,
    runtime_tree_sha256: runtime.sha256,
    runtime_source_files: runtime.files,
    suite_script: suitePath ? relative(repository, suitePath) : null,
  ...
  async function call(method, args = {}, options = {}) {
    ...
    try {
      ...
      const optionalError =
        !!response.isError &&
        options.allowError instanceof RegExp &&
        options.allowError.test(JSON.stringify(response.structuredContent ?? response.content));
      if (!!response.isError !== !!options.expectError && !optionalError)
      ...
      const entry = await record({
        ...
        method,
        args,
        status: optionalError ? 'skipped' : 'passed',
        level: response.isError ? 'native_assertion' : 'native_call',
        basis: response.isError ? 'explicit_assertion' : 'direct_input',
        direct_input_requirements: response.isError ? [] : observed,
        requirements: options.expectError
          ? [`tool:${method}`, ...(options.requirements ?? [])]
          : [...observed, ...(options.requirements ?? [])],
        elapsed_ms: Math.round(performance.now() - started),
        expected_error: !!options.expectError,
        ...(optionalError
          ? {
              reason: `Allowed runtime condition; processing did not succeed: ${JSON.stringify(response.structuredContent ?? response.content)}`,
            }
          : {}),
      ...
      return {
        ...
        result: response,
        images: response.content.filter((c) => c.type === 'image'),
        call_id: entry.id,
      };
    } catch (error) {
      await record({
        ...
        status: 'failed',
        level: 'native_call',
        requirements: [`tool:${method}`, ...(options.requirements ?? [])],
        elapsed_ms: Math.round(performance.now() - started),
        error: String(error),
      });
      throw error;
  ...
  if (capabilities.engine !== 'RapidRAW' || capabilities.protocol_version !== 1)
    throw new Error('A real compatible RapidRAW native binary is required');
  const inventory = requiredInventory(tools, capabilities);
  await writeFile(
    join(workspace, 'inventory.json'),
    JSON.stringify(
      { provenance, attribution_version: 'actual-schema-v2', tools, capabilities, requirements: inventory },
      null,
      2,
    ),
  );
  async function check(
    ...
    basis = 'explicit_assertion',
    source_call_ids,
  ) {
    if (!evidenceLevels.includes(level) || level === 'native_call' || level === 'visual_review')
      throw new Error('Use explicit native/pixel assertions; visual review requires a named reviewer and artifacts');
    try {
      if (basis === 'derived_state') {
        if (!requirements.length || !requirements.every((id) => inventory.includes(id)))
          throw new Error('Derived-state checks must name existing schema requirements');
        ...
        aggregateEvidence(inventory, [
          ...records,
          {
            evidence_format: 'rapidraw-native-evidence-v1',
            execution: 'real_native_mcp',
            id: 'validate-derived-source',
            type: 'check',
            status: 'passed',
            level,
            basis,
  ...
  return {
    ...
    async fixture(path, role, extra = {}) {
      const sidecar = `${path}.rrdata`;
      const sidecar_sha256 = await hashFile(sidecar).catch((error) => {
        if (error.code === 'ENOENT') return null;
        throw error;
      });
      const entry = { path, sha256: await hashFile(path), sidecar, sidecar_sha256, role, ...extra };
      fixtures.push(entry);
      return entry;
    },
    check,
    checkDerived: (name, requirements, sourceCallIds, fn, level = 'native_assertion') =>
      check(name, requirements, fn, level, 'derived_state', sourceCallIds),
    async skip(name, requirements, reason) {
      await record({ type: 'check', name, status: 'skipped', requirements, reason });
    },
    async close(error) {
      await client.close();
      if ((await hashFile(binary)) !== provenance.binary_sha256)
        await record({
          type: 'check',
          name: 'binary_preservation',
          requirements: ['tool:capabilities'],
          status: 'failed',
          error: 'Native binary changed during this run; do not combine evidence across builds',
        });
      if ((await runtimeSnapshot()).sha256 !== provenance.runtime_tree_sha256)
        await record({
          type: 'check',
          name: 'runtime_source_preservation',
          requirements: ['tool:capabilities'],
          status: 'failed',
          error: 'Runtime source changed during this run; rerun against a stable build',
        });
      for (const fixture of fixtures) {
        const current = await hashFile(fixture.path).catch(() => null);
        const sidecar = await hashFile(fixture.sidecar).catch((error) => {
          if (error.code === 'ENOENT') return null;
          throw error;
        });
        if (current !== fixture.sha256 || sidecar !== fixture.sidecar_sha256)
          await record({
            type: 'check',
            name: 'source_preservation',
            requirements: ['tool:open_photo'],
            status: 'failed',
            error: `Fixture or original sidecar changed: ${fixture.path}`,
          });
      }
      const report = aggregateEvidence(inventory, records);
      await writeFile(
        join(workspace, 'coverage.json'),
        JSON.stringify({ provenance, fixtures, error: error ? String(error) : undefined, ...report }, null, 2),
      );
        ...
        JSON.stringify(
          {
            provenance,
            fixtures,
            status: error || report.totals.failed ? 'failed' : 'passed',
      ...
      if (!error && report.totals.failed)
        throw new Error(
          'Native evidence contains failed checks or changed runtime/binary provenance; inspect summary.json and evidence.jsonl',
        );
      return report;
    },
  };
