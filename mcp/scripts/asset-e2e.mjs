...
const originalSidecar = await readFile(`${source}.rrdata`).catch((error) => {
...
const assertionCoverage = {
  ...
  local_ui_curve_create_update_remove: [
    'pixel_assertion',
    ['tool:mask_create', 'tool:mask_update', 'tool:mask_remove', 'tool:render'],
  ],
  cube_precision_and_spatial_exclusions: ['pixel_assertion', ['tool:export', 'parameter:export.format="cube"']],
  owned_lut_survives_external_move_and_restart: [
    'pixel_assertion',
    ['tool:apply_lut', 'tool:save_session', 'tool:render'],
  ],
  ...
  watermark_pixels_and_selective_exports: [
    'pixel_assertion',
    ['tool:export', 'parameter:export.watermark', 'parameter:export.export_masks=true'],
  ],
...
async function check(name, details) {
  ...
  const [level, requirements] = assertionCoverage[name] ?? [
    'native_assertion',
    name.startsWith('format_') ? ['tool:export', `parameter:export.format=${JSON.stringify(name.slice(7))}`] : [],
  ];
...
function watermarkPng() {
  ...
  for (let y = 0; y < 16; y++)
    for (let x = 0; x < 32; x++) {
      ...
      rows[offset] = 255;
      rows[offset + 1] = x < 16 ? 255 : 30;
      rows[offset + 2] = x < 16 ? 255 : 30;
      rows[offset + 3] = 255;
    }
...
async function inspectExternally(path) {
  try {
    const { stdout, stderr } = await exec(
      process.env.RAPIDRAW_MAGICK ?? 'magick',
      ['identify', '-format', '%w %h\n%[EXIF:Software]\n%[EXIF:GPSLatitude]\n%[EXIF:GPSLongitude]\n', path],
      { maxBuffer: 1024 * 1024 },
    );
    ...
    return {
      ...
      height,
      software: lines[1] ?? '',
      gps_latitude: lines[2] ?? '',
      gps_longitude: lines[3] ?? '',
    ...
    if (path.endsWith('.jxl')) {
      ...
      if (bytes[0] === 0xff && bytes[1] === 0x0a)
        return {
          raw_jxl_codestream: true,
          software: '',
          gps_latitude: '',
          gps_longitude: '',
          metadata_verification:
            'Raw JPEG XL codestream has no metadata container boxes; dimensions are independently decoded by the native jxl-oxide verifier.',
        };
    }
...
async function assertPixelDifference(first, second) {
  try {
    await exec(process.env.RAPIDRAW_MAGICK ?? 'magick', ['compare', '-metric', 'AE', first, second, 'null:']);
    assert.fail('Expected visibly different watermark output');
  } catch (error) {
    if (error.code === 1) {
      const changed = Number.parseFloat(String(error.stderr).trim());
      assert.ok(changed > 0);
      return changed;
    }
    throw error;
...
try {
  ...
  assert.ok(
    width >= 300 && height >= 300 && Math.max(width, height) <= 2048,
    'Use a small native photo export with both dimensions >=300',
  );
  ...
  const autoApplied = (
    await call('auto_adjust', { session_id, apply: true, expected_revision: originalState.revision })
  ).data;
  assert.equal(autoApplied.applied, true);
  assert.ok(autoApplied.revision > originalState.revision);
  await call('undo', { session_id });
  assert.deepEqual(
    (await call('get_session', { session_id, include_adjustments: true })).data.adjustments,
    originalState.adjustments,
  );
  ...
  await call('set_adjustments', {
    ...
    patch: {
      orientationSteps: 1,
      crop: { unit: 'px', x: 10, y: 20, width: 200, height: 300 },
      exposure: 0.27,
      contrast: 9,
      vibrance: 6,
    },
  ...
  assert.equal(cropped.data.width, 200);
  assert.equal(cropped.data.height, 300);
  assert.equal(cropped.data.rendered_width, 200);
  assert.equal(cropped.data.rendered_height, 300);
  const roi = await preview(session_id, 'native-roi', { region: { x: 15, y: 17, width: 80, height: 90 } });
  assert.equal(roi.data.width, 80);
  assert.equal(roi.data.height, 90);
  ...
  await check('orientation_crop_native_roi', {
    crop: [200, 300],
    region: [80, 90],
    region_scale: roi.data.coordinates.preview_to_rendered_scale,
  });
  ...
  if (presets.length) {
    ...
    assert.notEqual(
      (await preview(session_id, 'after-preset')).pixels,
      beforePreset.pixels,
      'Applying a nonzero installed preset must change rendered pixels',
    );
    if (presets[0].adjustments.enableNegativeConversion === false)
      assert.ok(presetApplied.warnings?.some((warning) => warning.includes('inactive legacy')));
    await call('undo', { session_id });
    assert.deepEqual(
      (await call('get_session', { session_id, include_adjustments: true })).data.adjustments,
      before.adjustments,
    ...
    skipped.push({
      name: 'existing_preset_apply',
      reason: 'No installed presets available; installed assets were not mutated.',
    });
  ...
  const localCurve = (
    await call('mask_create', {
      session_id,
      type: 'all',
      name: 'Local point curve verification',
      parameters: {},
      adjustments: {
        curveMode: 'point',
        pointCurves: {
  ...
  assert.notEqual(
    createdLocalCurve.pixels,
    beforeLocalCurve.pixels,
    'Local UI point curves must change pixels when the mask is created without explicit native curves',
  );
  await call('mask_update', {
    session_id,
    mask_id: localCurve.mask_id,
    patch: {
      adjustments: {
        pointCurves: {
          luma: [
  ...
  assert.notEqual(
    updatedLocalCurve.pixels,
    createdLocalCurve.pixels,
    'Updating only local pointCurves must recompile previously generated native curves',
  );
  await call('mask_remove', { session_id, mask_id: localCurve.mask_id });
  assert.equal(
    (await preview(session_id, 'removed-local-curve')).pixels,
    beforeLocalCurve.pixels,
    'Removing the local curve mask must restore the original rendered pixels',
  );
  await check('local_ui_curve_create_update_remove', {
    created_luma_endpoint: 180,
    updated_luma_endpoint: 230,
    removed_restores_baseline: true,
  });
  ...
  await check('cube_precision_and_spatial_exclusions', {
    ...cubeStats,
    excluded_adjustments: cube.excluded_adjustments,
  });
  ...
  assert.notEqual(ownedLut, cubePath);
  assert.ok(ownedLut.includes(`/sessions/${session_id}/assets/`));
  ...
  const inheritedSidecar = {
    version: 1,
    rating: 2,
    tags: ['asset-e2e'],
    futureEnvelopeField: { preserved: true },
    adjustments: { lutPath: movedCube, lutIntensity: 70 },
  };
  ...
  await check('source_sidecar_lut_materialization', {
  ...
  for (const [format, extension] of [
    ...
    const output = (
      await call('export', {
        ...
        path,
        format,
        resize: { mode: 'width', value: 240 },
        keep_metadata: true,
        strip_gps: true,
        ...(['png', 'tiff'].includes(format) ? { bit_depth: 16 } : {}),
      })
    ).data;
    assert.equal(output.width, 240);
    assert.equal(output.height, Math.round((height * 240) / width));
    ...
    if (external) {
      if (!external.raw_jxl_codestream) {
        assert.equal(external.width, output.width);
        assert.equal(external.height, output.height);
      }
      assert.equal(external.gps_latitude, '');
      assert.equal(external.gps_longitude, '');
      assert.equal(
        output.metadata_applied,
        external.software.includes('RapidRAW'),
        `${format}: metadata_applied must match exported Software tag`,
      );
    }
    await check(`format_${format}`, {
      path,
      metadata_applied: output.metadata_applied,
      verification: output.verified,
      external,
    });
  }
  for (const [mode, value, expectedWidth, expectedHeight] of [
    ['height', 180, Math.round((width * 180) / height), 180],
    [
      'longEdge',
      220,
      Math.round((width * 220) / Math.max(width, height)),
      Math.round((height * 220) / Math.max(width, height)),
    ],
    [
      'shortEdge',
      120,
      Math.round((width * 120) / Math.min(width, height)),
      Math.round((height * 120) / Math.min(width, height)),
    ],
    ...
    const output = (
      await call('export', {
        session_id,
        path: join(workspace, 'exports', `${stamp}-resize-${mode}.jpg`),
        format: 'jpeg',
        resize: { mode, value },
      })
    ).data;
  ...
  await call(
    ...
    {
      session_id,
      path: join(workspace, 'exports', `${stamp}-conflict.jpg`),
      long_edge: 100,
  ...
  const watermarkPath = join(workspace, `${stamp}-watermark.png`);
  await writeFile(watermarkPath, watermarkPng());
  const plainPath = join(workspace, 'exports', `${stamp}-unmarked.png`),
    markedPath = join(workspace, 'exports', `${stamp}-marked.png`);
  await call('export', {
    session_id,
    path: plainPath,
    format: 'png',
    bit_depth: 8,
    resize: { mode: 'width', value: 320 },
  });
  const marked = (
    await call('export', {
      session_id,
      ...
      bit_depth: 8,
      resize: { mode: 'width', value: 320 },
      watermark: { path: watermarkPath, anchor: 'bottomRight', scale: 25, spacing: 3, opacity: 75 },
      export_masks: true,
    })
  ).data;
  assert.equal(marked.masks.length, 1);
  assert.equal(marked.masks[0].mask_id, mask.mask_id);
  assert.ok(marked.masks[0].coverage.nonzero_fraction > 0);
  await stat(marked.masks[0].image_path);
  await stat(marked.masks[0].alpha_path);
  ...
  await call(
    'export',
    { session_id, path: join(workspace, 'recipes', `${stamp}-wrong-category.jpg`), format: 'jpeg' },
    true,
  );
  await call('save_recipe', { session_id, path: join(workspace, 'exports', `${stamp}-wrong-category.json`) }, true);
  const protectedPath = join(workspace, 'exports', `${stamp}-protected-source${extname(source) || '.jpg'}`);
  await copyFile(source, protectedPath);
  ...
  await call(
    'export',
    { session_id: protectedSession.session_id, path: protectedPath, format: 'jpeg', overwrite: true },
    true,
  );
  ...
  const after = await readFile(`${source}.rrdata`).catch((error) => {
    if (error.code === 'ENOENT') return null;
    throw error;
  });
