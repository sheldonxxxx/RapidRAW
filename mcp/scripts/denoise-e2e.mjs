...
function writeFloatTiff() {
  ...
  const tags = [
    ...
    [277, 3, 1, 3],
    [278, 4, 1, height],
    [279, 4, 1, width * height * 12],
    [284, 3, 1, 1],
    [339, 3, 3, formats],
  ];
  tags.forEach(([tag, type, n, value], index) => {
    const at = 10 + index * 12;
    buffer.writeUInt16LE(tag, at);
    buffer.writeUInt16LE(type, at + 2);
    buffer.writeUInt32LE(n, at + 4);
    buffer.writeUInt32LE(value, at + 8);
  });
  for (let c = 0; c < 3; c++) {
    buffer.writeUInt16LE(32, bits + c * 2);
    buffer.writeUInt16LE(3, formats + c * 2);
  }
...
function readFloatTiff(buffer) {
  ...
  for (let i = 0; i < buffer.readUInt16LE(ifd); i++) {
    const at = ifd + 2 + i * 12,
      tag = buffer.readUInt16LE(at),
      type = buffer.readUInt16LE(at + 2),
      n = buffer.readUInt32LE(at + 4),
      size = type === 3 ? 2 : 4;
    const offset = n * size <= 4 ? at + 8 : buffer.readUInt32LE(at + 8);
    tags.set(
      tag,
      Array.from({ length: n }, (_, j) =>
        size === 2 ? buffer.readUInt16LE(offset + j * size) : buffer.readUInt32LE(offset + j * size),
      ),
    );
  }
  assert.equal(tags.get(259)[0], 1, 'Fixture comparison expects uncompressed TIFF');
  assert.equal(tags.get(258)[0], 32);
  assert.equal(tags.get(339)[0], 3);
  ...
  return {
    ...
    values: Array.from({ length: bytes.length / 4 }, (_, i) => bytes.readFloatLE(i * 4)),
  };
}
...
async function connect() {
  if (process.env.RAPIDRAW_COVERAGE === '1') {
    if (coverage) await coverage.reconnect();
      ...
      coverage = await createNativeHarness({ suite: 'denoise-linear-domain', workspace, binary, timeout: 600000 });
      await coverage.fixture(
        fixture,
        'Synthetic float32 TIFF with isolated session RAW flag; not a photographic RAW inference benchmark',
      );
    }
...
async function call(method, args) {
  ...
  const result = coverage
    ? (await coverage.call(method, args)).result
    : await client.callTool({ name: `rapidraw_${method}`, arguments: args }, { timeout: 600000 });
  assert.ok(!result.isError, JSON.stringify(result));
  records.push({ method, args, elapsed_ms: Math.round(performance.now() - started), data: result.structuredContent });
  return result;
}
async function render(id) {
  return (await call('render', { session_id: id, format: 'png' })).content.find((c) => c.type === 'image').data;
}
try {
  await connect();
  ...
  session.is_raw = true;
  session.history[0].metadata.mcpSourceDomain = 'linear-raw';
  ...
  const zero = (
    await call('denoise', { session_id: session.id, method: 'ai', intensity: 0, expected_revision: before.revision })
  ).structuredContent;
  assert.equal(zero.session_id, session.id);
  assert.equal(zero.revision, before.revision);
  assert.equal(zero.changed, false);
  assert.deepEqual(zero.adjustments, before.adjustments);
  assert.equal(await render(session.id), baseline);
  if (coverage)
    await coverage.check(
      'zero_intensity_preserves_state_and_render',
      ['tool:denoise', 'parameter:denoise.intensity'],
      async () => ({ changed: false, identical_render: true, fixture: 'synthetic_float_tiff_marked_raw' }),
      'pixel_assertion',
    );
  ...
  for (const method of ['bm3d', 'ai']) {
    const denoised = (
      await call('denoise', { session_id: session.id, method, intensity: 25, expected_revision: before.revision })
    ).structuredContent;
    assert.equal(denoised.is_raw, true);
    assert.equal(denoised.source_domain_preserved, true);
    ...
    const max = Math.max(...image.values);
    assert.ok(max > 1.3, 'Linear highlight headroom was lost');
    ...
    for (let band = 0; band < 3; band++) {
      let delta = 0,
        ...
        originalSquare = 0;
      for (let y = 16; y < 48; y++)
        for (let x = band * 32 + 10; x < band * 32 + 22; x++)
          for (let c = 0; c < 3; c++) {
            const i = (y * image.width + x) * 3 + c;
            delta += image.values[i] - original.values[i];
            n++;
            sum += image.values[i];
            square += image.values[i] ** 2;
            originalSum += original.values[i];
            originalSquare += original.values[i] ** 2;
          }
      means.push(delta / n);
      noise.push({
        before: Math.sqrt(Math.max(0, originalSquare / n - (originalSum / n) ** 2)),
        after: Math.sqrt(Math.max(0, square / n - (sum / n) ** 2)),
      });
    }
    assert.ok(
      means.every((d) => Math.abs(d) < 0.025),
      `${method} gratuitous tone drift ${means}`,
    );
    if (method === 'bm3d')
      assert.ok(
        noise.every((n) => n.after < n.before * 0.95),
        'BM3D did not reduce the flat-field fixture noise',
      );
    const preview = await render(denoised.session_id);
    await call('save_session', { session_id: denoised.session_id });
    await client.close();
    await connect();
    const restored = (await call('get_session', { session_id: denoised.session_id, include_adjustments: true }))
      .structuredContent;
    assert.equal(restored.is_raw, true);
    assert.deepEqual(restored.adjustments, before.adjustments);
    assert.equal(await render(denoised.session_id), preview, 'Rendering changed after reconnect');
    results.push({
      method,
      session_id: denoised.session_id,
      max_linear_value: max,
      mean_source_deltas: means,
      flat_field_noise: noise,
    });
    if (coverage)
      await coverage.check(
        `linear_domain_${method}_headroom_drift_and_persistence`,
        ['tool:denoise', `parameter:denoise.method=${JSON.stringify(method)}`],
        async () => results.at(-1),
        'pixel_assertion',
      );
  }
  ...
  if (process.env.RAPIDRAW_MASK_TEST_IMAGE) {
    const photo = (await call('open_photo', { path: process.env.RAPIDRAW_MASK_TEST_IMAGE, inherit_sidecar: false }))
      .structuredContent;
    const generated = (
      await call('mask_generate', {
        session_id: photo.session_id,
        kind: 'foreground',
        adjustments: { curveMode: 'parametric', parametricCurve: { luma: { shadows: 50 } } },
      })
    ).structuredContent;
    const state = (await call('get_session', { session_id: photo.session_id, include_adjustments: true }))
      .structuredContent;
    const local = state.adjustments.masks.find((mask) => mask.id === generated.mask_id).adjustments;
    assert.ok(
      local.curves.luma.some((point) => Math.abs(point.x - point.y) > 0.01),
      'AI local parametric curve was not compiled',
    );
    await render(photo.session_id);
  }
  const elapsed_ms = Math.round(performance.now() - started);
  await writeFile(
    join(workspace, 'denoise-domain-evidence.json'),
    JSON.stringify({ status: 'passed', elapsed_ms, results, records }, null, 2),
  );
