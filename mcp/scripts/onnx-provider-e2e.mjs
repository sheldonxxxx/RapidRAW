...
for (const [name, provider] of Object.entries(expected)) {
  assert.ok(name.endsWith('.onnx') && !/[\\/]/.test(name) && ['cpu', 'cuda'].includes(provider), 'Expected providers must map model basenames to cpu or cuda');
}
const tolerance = { mean_absolute: 0.01, p99_absolute: 0.05, ...JSON.parse(process.env.RAPIDRAW_ONNX_TOLERANCE ?? '{}') };
...
const report = {
  schema: 'rapidraw-onnx-provider-v1', status: 'running', requested_provider: requested,
  provider_explicitly_set: process.env.RAPIDRAW_ONNX_PROVIDER !== undefined,
  platform: process.platform, source, source_sha256: await hashFile(source), runs,
  operations: {}, artifacts: {}, comparisons: {}, tolerance, baseline: baselinePath ?? null,
  timing_scope: 'Native MCP operation wall time including preparation and serialization; excludes separate open/render/diagnostic calls. First calls include lazy initialization where applicable. Warm means model sessions retained, with a fresh source working path for each operation.',
...
async function fresh() {
  ...
  assert.ok(opened.working_path && !workingPaths.has(opened.working_path), 'Each inference must receive a new path to avoid SAM/depth cache reuse');
...
async function timed(label, iteration, method, args) {
  ...
  const start = performance.now(), response = await h.call(method, args);
...
function difference(a, b) {
  const result = pixelDifference(a, b), histogram = new Uint32Array(256);
  for (let i = 0; i < a.pixels.length; i++) histogram[Math.round(Math.abs(a.pixels[i] - b.pixels[i]) * 255)]++;
  let count = 0, p99 = 0;
  for (; p99 < 255; p99++) { count += histogram[p99]; if (count >= a.pixels.length * 0.99) break; }
...
async function artifact(name, response, expectedDimensions) {
  ...
  const bytes = Buffer.from(response.images[0].data, 'base64'), decoded = decodePng(bytes);
  ...
  const relativePath = `artifacts/${name}.png`, path = join(workspace, relativePath);
  await writeFile(path, bytes, { flag: 'wx' });
  report.artifacts[name] = { path: relativePath, sha256: hashBytes(bytes), width: decoded.width, height: decoded.height, bit_depth: decoded.bitDepth };
  if (baseline) {
    ...
    await persist();
    assert.ok(metrics.mean_absolute <= tolerance.mean_absolute, `${name}: MAE ${metrics.mean_absolute} exceeds ${tolerance.mean_absolute}`);
    assert.ok(metrics.p99_absolute <= tolerance.p99_absolute, `${name}: p99 ${metrics.p99_absolute} exceeds ${tolerance.p99_absolute}`);
  }
  return decoded;
}
...
function inpaintTensor(region) {
  ...
  const cropWidth = Math.min(dimensions.width - 1, region.x + region.width - 1 + padX) - Math.max(0, region.x - padX) + 1;
  const cropHeight = Math.min(dimensions.height - 1, region.y + region.height - 1 + padY) - Math.max(0, region.y - padY) + 1;
...
try {
  ...
  const exported = (await h.call('export', { session_id: original.session_id, path: 'onnx-fixture.png', format: 'png', bit_depth: 8, long_edge: 1024, color_profile: 'none', keep_metadata: false })).data;
  ...
  assert.ok(Math.max(pixels.width, pixels.height) === 1024 && Math.min(pixels.width, pixels.height) >= 448, 'Use a RAW with enough pixels and an aspect ratio suitable for both inpaint crop sizes');
  assert.deepEqual([exported.width, exported.height], [pixels.width, pixels.height]);
  report.fixture = { dimensions, sha256: await hashFile(seed), pixels_sha256: hashBytes(Buffer.from(pixels.pixels.buffer)) };
  await h.fixture(seed, '1024px native rendering of the original RAW; inference uses display-referred pixels', { parent_sha256: report.source_sha256 });
  report.before = await snapshot();
  assert.deepEqual(report.before.onnx_execution.models, {}, 'Use a fresh native process; model diagnostics should be uninitialized before inference');
  for (const kind of ['masks', 'inpaint', 'denoise']) {
    assert.ok(report.before.groups[kind].assets.every(asset => asset.verified || asset.installed_app_copy_available), `Install ${kind} assets before running; this suite never downloads models`);
  }
  report.model_sha256 = Object.fromEntries(Object.values(report.before.groups).flatMap(group => group.assets.map(asset => [asset.name, asset.expected_sha256])));
  ...
  if (baseline) {
    ...
    assert.equal(baseline.fixture.pixels_sha256, report.fixture.pixels_sha256, 'Compare providers using identical native fixture pixels');
    assert.deepEqual(baseline.model_sha256, report.model_sha256, 'Baseline must use identical model assets');
  }
  for (const name of Object.keys(expected)) assert.ok(Object.hasOwn(report.model_sha256, name), `Unknown expected model: ${name}`);
  ...
  for (const kind of ['subject', 'foreground', 'sky', 'depth']) {
    for (let iteration = 0; iteration < runs; iteration++) {
      ...
      const generated = await timed(`mask-${kind}`, iteration, 'mask_generate', {
        session_id: session.session_id, expected_revision: session.revision, kind,
        ...(kind === 'subject' ? { region: { x: dimensions.width * 0.1, y: dimensions.height * 0.05, width: dimensions.width * 0.8, height: dimensions.height * 0.9 } } : {}),
        parameters: kind === 'depth' ? { minDepth: 20, maxDepth: 80, minFade: 10, maxFade: 10, feather: 0 } : { grow: 0, feather: 0 },
      });
      assert.equal(generated.data.mask_statistics.empty, false, `Choose a fixture with a useful ${kind} selection`);
      assert.deepEqual([generated.data.mask_statistics.width, generated.data.mask_statistics.height], [dimensions.width, dimensions.height]);
  ...
  for (const tensor of [448, 768]) {
    for (let iteration = 0; iteration < runs; iteration++) {
      const session = await fresh(), radius = tensor === 448 ? 54 : 128;
      const retouched = await timed(`inpaint-${tensor}`, iteration, 'retouch', {
        session_id: session.session_id, expected_revision: session.revision, mode: 'inpaint',
        sub_masks: [{ id: `inpaint-${tensor}-${iteration}`, type: 'radial', visible: true, mode: 'additive', parameters: {
          centerX: Math.floor(dimensions.width / 2), centerY: Math.floor(dimensions.height / 2), radiusX: radius, radiusY: radius, rotation: 0, feather: 0,
        } }],
      });
      ...
      assert.ok(region.x >= 0 && region.y >= 0 && region.x + region.width <= dimensions.width && region.y + region.height <= dimensions.height);
      assert.equal(inpaintTensor(region), tensor, 'Patch bounds must exercise the requested native LaMa tensor size');
      report.operations[`inpaint-${tensor}`].crop = { affected_region: region, tensor_dimensions_from_native_crop_policy: [tensor, tensor] };
      if (iteration === runs - 1) {
        await artifact(`inpaint-${tensor}-mask`, retouched, [region.width, region.height]);
        await artifact(`inpaint-${tensor}-result`, await render(session.session_id), [dimensions.width, dimensions.height]);
        await artifact(`inpaint-${tensor}-affected-region`, await render(session.session_id, region), [region.width, region.height]);
      }
      await h.call('close_session', { session_id: session.session_id });
    }
  ...
  for (let iteration = 0; iteration < runs; iteration++) {
    const session = await fresh();
    const result = await timed('denoise-ai', iteration, 'denoise', { session_id: session.session_id, expected_revision: session.revision, method: 'ai', intensity: 50 });
    ...
    assert.equal(result.data.is_raw, false, 'The bounded test uses a native display-referred export, not full RAW denoise');
    ...
    if (iteration === runs - 1) await artifact('denoise-ai-result', await render(result.data.session_id), [dimensions.width, dimensions.height]);
    for (const session_id of [session.session_id, result.data.session_id]) await h.call('close_session', { session_id });
  }
  report.operations['denoise-ai'].native_tile_policy = { tensor: [1, 3, 504, 504], stride: 480, overlap: 6, intensity: 50, scope: 'Current balanced native tile policy; this is not an operator trace' };
  report.after = await snapshot();
  await h.check('all_seven_model_sessions_have_explicit_provider_diagnostics', ['tool:models', 'tool:mask_generate', 'tool:retouch', 'tool:denoise'], async () => {
    for (const [name, sha256] of Object.entries(report.model_sha256)) {
      ...
      const expectedProvider = expected[name] ?? (requested === 'cpu' || process.platform !== 'linux' ? 'cpu' : undefined);
      if (expectedProvider) assert.equal(status.session_provider, expectedProvider, `${name}: provider differs from expectation`);
      assert.equal(await hashFile(join(workspace, 'models', name)), sha256);
    }
    return { actual: report.after.onnx_execution.models, expected, successful_operations: Object.keys(report.operations), provider_scope: report.provider_scope };
  });
  assert.equal(await hashFile(source), report.source_sha256);
  report.status = 'passed';
} catch (error) {
  failure = error; report.status = 'failed'; report.error = String(error);
  try { report.after_failure = await snapshot(); } catch (diagnosticError) { report.diagnostics_error = String(diagnosticError); }
} finally {
  try { await h.close(failure); } catch (error) { failure ??= error; report.status = 'failed'; report.error ??= String(error); }
  await persist();
}
if (failure) throw failure;
console.log(JSON.stringify({ status: report.status, results: resultPath, timings: Object.fromEntries(Object.entries(report.operations).map(([name, operation]) => [name, { first_ms: operation.first_ms, warm_median_ms: operation.warm_median_ms }])) }, null, 2));
