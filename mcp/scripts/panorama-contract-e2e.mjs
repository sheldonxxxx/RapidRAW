...
try {
  if (process.env.RAPIDRAW_PANORAMA_LIMIT_MANIFEST) {
    const manifest = await validatePhotoManifest(JSON.parse(await readFile(resolve(process.env.RAPIDRAW_PANORAMA_LIMIT_MANIFEST), 'utf8')));
    assert.equal(manifest.groups.length, 1); const group = manifest.groups[0]; assert.equal(group.kind, 'panorama');
    for (const source of group.sources) await h.fixture(source.path, 'genuine capture group whose projected canvas exceeds 100MP', { capture_group: group.capture_group, partition: group.partition });
    await h.check('panorama_rejects_over_limit_projected_capture_before_allocation', requirements, async () => {
      const result = await h.call('merge', { kind: 'panorama', paths: group.sources.map((source) => source.path) }, { expectError: true });
      assert.match(JSON.stringify(result.data), /PANORAMA_CANVAS_TOO_LARGE/);
      assert.equal(result.data?.session_id, undefined);
      return { source_count: group.sources.length, maximum_canvas_megapixels: 100, projected_canvas_rejected: true, photographic_quality: 'no stitched output; limit assertion only' };
    });
  }
  const directory = join(workspace, 'fixtures'); await mkdir(directory, { recursive: true });
  function scene(x, y) {
    ...
    const value = 112 + ((hash >>> 0) % 32); return [value, value, value];
  }
  const panels = [];
  for (const [i, offset] of [0, 150, 300].entries()) {
    const path = join(directory, `low-contrast-panel-${i + 1}.png`);
    await writeFile(path, fixturePng(600, 420, (x, y) => scene(x + offset, y)), { flag: 'wx' });
    await h.fixture(path, 'procedural overlapping panel; not a photographic capture'); panels.push(path);
  }
  const blank = join(directory, 'blank.png'); await writeFile(blank, fixturePng(600, 420, () => [128, 128, 128]), { flag: 'wx' });
  ...
  await h.check('panorama_rejects_disconnected_source_instead_of_silent_partial_result', requirements, async () => {
    const result = await h.call('merge', { kind: 'panorama', paths: [panels[0], panels[2], blank] }, { expectError: true });
  ...
  await h.check('low_contrast_panorama_preserves_all_sources_and_restart_pixels', [...requirements, 'tool:save_session', 'tool:render'], async () => {
    const merged = (await h.call('merge', { kind: 'panorama', paths: panels })).data;
    const session_id = merged.session_id; assert.ok(session_id);
    ...
    assert.ok(png.pixels.some((v) => v > 0.4), 'Expected actual nonempty image samples');
    await writeFile(join(workspace, 'panorama.png'), Buffer.from(before.images[0].data, 'base64'), { flag: 'wx' });
    await h.call('save_session', { session_id }); await h.reconnect();
    const after = await h.call('render', { session_id, format: 'png' });
    assert.deepEqual(after.images, before.images);
    return { width: png.width, height: png.height, sources: 3, restart_pixels_equal: true, photographic_quality: 'procedural fixture; not assessed' };
  }, 'pixel_assertion');
