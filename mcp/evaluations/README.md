...
`evaluation.xml` contains ten independent questions about realistic editing decisions: temporary comparisons, named reference recovery, native gradient coordinates, mask diagnostics, captured job inputs and cooperative cancellation. Answers were checked against the built native bridge through the real MCP server, using only `capabilities`, `get_engine_settings`, `tools/list` and the workflow resource. No photos or edits are needed to answer them.

`reference.json` freezes the returned capabilities, full native edit schema, tool discovery and workflow as **fixture version 2**. Its workspace path is normalized to `/fixture/rapidraw`; other contract data is preserved. This is an interface-understanding evaluation, not a claim that a model has completed a successful artistic edit. The XML has reference answers; no external language-model benchmark has been run.
