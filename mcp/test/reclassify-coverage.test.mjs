...
const exec = promisify(execFile),
  script = fileURLToPath(new URL('../scripts/reclassify-coverage.mjs', import.meta.url));
async function fixture(t, status = 'passed') {
  const root = await mkdtemp(join(tmpdir(), 'rapidraw-historical-attribution-unit-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const input = join(root, 'original'),
    output = join(root, 'corrected');
  await mkdir(input);
  const tools = [
    {
  ...
  const records = [
    {
      ...
      args: {
        mode: 'clone',
        sub_masks: [{ id: 'stroke', type: 'clone', visible: true, mode: 'additive', parameters: {} }],
      },
      requirements: [
        'tool:retouch',
        'parameter:retouch.mode',
        'parameter:retouch.mode="clone"',
        'parameter:retouch.sub_masks',
        'parameter:retouch.sub_masks[]<clone>.id',
        'parameter:retouch.sub_masks[]<clone>.type',
        'parameter:retouch.sub_masks[]<clone>.visible',
        'parameter:retouch.sub_masks[]<clone>.visible=true',
        'parameter:retouch.sub_masks[]<clone>.mode',
        'parameter:retouch.sub_masks[]<clone>.mode="additive"',
        'parameter:retouch.sub_masks[]<clone>.parameters',
      ],
      status: 'passed',
      level: 'native_call',
    ...
    },
    {
      ...
      requirements: ['parameter:retouch.mode="generative"'],
      status: 'skipped',
...
test('historical correction repairs seven recorded generic fields without new calls, defaults or quality credit', async (t) => {
  const f = await fixture(t),
    filenames = await readdir(f.input);
  ...
  assert.deepEqual(
    result.requirements.map((row) => row.id),
    f.requirements,
  );
  assert.equal(result.reclassification.new_native_calls, 0);
  assert.equal(result.reclassification.reconstructed_successful_calls, 1);
  ...
  for (const id of [
    'parameter:retouch.enabled=true',
    'parameter:retouch.mode="generative"',
    'adjustment:lensBlurEnabled=true',
  ])
    assert.deepEqual(result.requirements.find((row) => row.id === id).levels, [], id);
...
test('historical correction refuses failed runs, missing schemas and changed inventory before writing output', async (t) => {
  for (const mode of ['failed', 'missing-schema', 'changed-inventory'])
    await t.test(mode, async (t) => {
      const f = await fixture(t, mode === 'failed' ? 'failed' : 'passed');
      if (mode !== 'failed') {
        const path = join(f.input, 'inventory.json'),
          data = JSON.parse(await readFile(path, 'utf8'));
        if (mode === 'missing-schema') delete data.tools;
        else data.requirements.push('tool:invented');
...
test('historical correction never writes into a source run and never overwrites a previous correction', async (t) => {
  const f = await fixture(t);
  await assert.rejects(run({ ...f, output: join(f.input, 'correction') }), (error) => {
    assert.match(error.stderr, /outside original run/);
    return true;
  });
  await run(f);
  const hash = await hashFile(join(f.output, 'reclassification.json'));
