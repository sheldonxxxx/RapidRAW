...
lines.on('line', async (line) => {
  ...
  if (busy) {
    process.stdout.write(
      `${JSON.stringify({ id, error: { code: 'CONCURRENT', message: 'Concurrent native request' } })}\n`,
    );
    return;
  }
  busy = true;
  ...
  if (params.session_id === 'wrong_id') {
    process.stdout.write(`${JSON.stringify({ id: id + 1, result: {} })}\n`);
    return;
  }
  if (params.session_id === 'eof') {
    process.stdout.end();
    return;
  }
  ...
  if (method === 'batch_export')
    result = {
      ...
      failed: 1,
      results: [
        { ok: true, result: { path: '/tmp/ok.jpg' } },
        { ok: false, error: 'SESSION_NOT_FOUND', path: '/tmp/bad.jpg' },
      ],
    };
  if (method === 'render')
    result = {
  ...
  if (
    (params.session_id === 'oversized-images' && method === 'render') ||
    (params.session_id === 'oversized-mutation' && method === 'mask_generate')
  ) {
    ...
    result = {
      ...
      images: [
        { label: 'Photograph', data, mimeType: 'image/png' },
        { label: 'Selection', data, mimeType: 'image/png' },
      ],
    };
    if (method === 'mask_generate') {
      mutationCount++;
      result.mask_id = 'mask-created';
      result.revision = 3;
    }
  }
  if (params.session_id === 'oversized-state' && method === 'get_session' && params.include_adjustments)
    result.adjustments = { description: 'é'.repeat(3 * 1024 * 1024) };
  if (params.session_id === 'oversized-resource' && method === 'get_session' && params.include_adjustments)
    result.adjustments = { description: 'é'.repeat(5 * 1024 * 1024) };
  ...
  if (params.session_id === 'oversized-error' && method === 'render') {
    process.stdout.write(
      `${JSON.stringify({ id, error: { code: 'NATIVE_FAILURE', message: 'x'.repeat(12 * 1024 * 1024) } })}\n`,
    );
  } else if (method === 'set_adjustments' && params.patch?.bad) {
    process.stdout.write(
      `${JSON.stringify({ id, error: { code: 'INVALID_ADJUSTMENT', message: 'Unknown adjustment bad; read the edit schema.' } })}\n`,
    );
  } else {
