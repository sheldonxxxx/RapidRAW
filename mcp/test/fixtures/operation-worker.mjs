...
for await (const line of createInterface({ input: process.stdin })) {
  ...
  if (method === 'export_session_bundle') {
    const path = join(workspace, 'bundles', 'result');
    await mkdir(path, { recursive: true });
    result = { path, manifest_sha256: '0'.repeat(64) };
  }
  if (
    ['merge', 'export', 'negative_convert', 'mask_generate', 'generate_depth', 'retouch', 'enhance'].includes(method)
  ) {
    if (process.env.FAKE_OPERATION_STALL === '1') {
      await new Promise(() => {});
    }
    await new Promise((resolve) => setTimeout(resolve, 20));
    const settings = await readFile(join(workspace, 'engine-settings.json'), 'utf8')
      .then(JSON.parse)
      .catch(() => null);
    const models = {};
    for (const name of await readdir(join(workspace, 'models')).catch(() => []))
      models[name] = await readFile(join(workspace, 'models', name), 'utf8');
...
if (process.env.FAKE_CLOSE_DELAY_MS)
  await new Promise((resolve) => setTimeout(resolve, Number(process.env.FAKE_CLOSE_DELAY_MS)));
