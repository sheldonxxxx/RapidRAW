...
test('schema inventory includes tools, boolean modes, nested adjustments and discriminated mask parameters', () => {
  const tools = [
    {
      name: 'rapidraw_example',
      inputSchema: { properties: { mode: { enum: ['one', 'two'] }, enabled: { type: 'boolean' } } },
    },
  ];
  const capabilities = {
    adjustment_schema: {
      properties: {
        ...
        masks: {
          items: {
            oneOf: [
              { properties: { type: { const: 'flow' }, flow: { type: 'number' } } },
              { properties: { type: { const: 'brush' }, feather: { type: 'number' } } },
            ],
          },
        },
      },
  ...
  for (const id of [
    'tool:example',
    'parameter:example.mode="two"',
    'parameter:example.enabled=false',
    'adjustment:exposure',
    'adjustment:masks[]<flow>.flow',
    'adjustment:masks[]<brush>.feather',
  ])
...
test('runtime provenance includes actual engine/server code but separates suite and documentation edits', () => {
  for (const path of [
    'src-tauri/src/mcp_bridge/portable.rs',
...
test('mock calls, skipped tests and native errors never earn native or pixel credit', () => {
  ...
  const result = aggregateEvidence(
    ['tool:render', 'adjustment:exposure'],
    [
  ...
  assert.equal(result.requirements[0].failures.length, 1);
  assert.equal(result.totals.pixel_assertion, 0);
  assert.equal(result.totals.untested, 1);
});
test('pixel assertions credit only explicit requirements and never infer visual review', () => {
  const result = aggregateEvidence(
    ['tool:render', 'adjustment:exposure', 'adjustment:sharpness'],
    [
      {
        evidence_format: 'rapidraw-native-evidence-v1',
        execution: 'real_native_mcp',
        requirements: ['adjustment:exposure'],
        id: '1',
        level: 'pixel_assertion',
        status: 'passed',
      },
    ],
  );
  assert.equal(result.totals.pixel_assertion, 1);
  assert.equal(result.totals.visual_review, 0);
  assert.deepEqual(result.requirements[2].levels, []);
});
test('actual arguments grant observed-parameter call credit without inferring defaults', () => {
                  ...
                  oneOf: [
                    {
                      properties: {
                        type: { const: 'flow' },
                        mode: { enum: ['intersect'] },
                        parameters: { properties: { lines: { type: 'array' } } },
                      },
                    },
                    { properties: { type: { const: 'brush' } } },
                  ],
                },
              },
            },
  ...
  const observed = observedRequirements(
    'set_adjustments',
    {
      session_id: 'fixture',
      patch: { exposure: 1, masks: [{ subMasks: [{ type: 'flow', mode: 'intersect', parameters: { lines: [] } }] }] },
    },
    { capabilities },
  );
  for (const id of [
    'tool:set_adjustments',
    'parameter:set_adjustments.session_id',
    'adjustment:exposure',
    'adjustment:masks[].subMasks[]<flow>.mode="intersect"',
  ])
    assert.ok(observed.includes(id), id);
...
test('generic retouch schema records real submask fields without inventing native discriminators', () => {
  ...
  const actual = observedRequirements(
    'retouch',
    { sub_masks: [{ id: 'clone-stroke', type: 'clone', visible: true, mode: 'additive', parameters: { sourceX: 3 } }] },
    { tools },
  );
  for (const suffix of ['id', 'type', 'visible', 'visible=true', 'mode', 'mode="additive"', 'parameters'])
    assert.ok(actual.includes(`parameter:retouch.sub_masks[].${suffix}`), suffix);
...
test('discriminated array variants never share credit and generic records remain generic', () => {
  const properties = {
    items: {
      oneOf: ['flow', 'brush'].map((kind) => ({ properties: { type: { const: kind }, amount: { type: 'number' } } })),
    },
  };
  const tools = [
    {
      name: 'rapidraw_example',
      inputSchema: { properties: { components: properties, arbitrary: { type: 'object' } } },
    },
  ];
  const actual = observedRequirements(
    'example',
    {
      components: [{ type: 'flow', amount: 50 }],
      arbitrary: { type: 'brush', mode: 'intersect', parameters: { amount: 20 } },
    },
    { tools },
  );
          ...
          components: {
            anyOf: [
              { type: 'null' },
              {
                type: 'array',
                items: {
                  oneOf: ['flow', 'brush'].map((kind) => ({
                    properties: { type: { const: kind }, amount: { type: 'number' } },
                  })),
                },
              },
            ],
...
test('derived state requires explicit successful-call assertions and stays separate from direct input', () => {
  ...
  const call = {
    ...
    basis: 'direct_input',
    level: 'native_call',
    requirements: ['tool:generate_depth'],
    result: { lensBlurEnabled: true },
  };
  const ids = ['tool:generate_depth', 'adjustment:lensBlurEnabled=true', 'adjustment:lensBlurShape="ring"'];
  assert.deepEqual(
    aggregateEvidence(ids, [call]).requirements[1].levels,
    [],
    'Returned state alone must grant no evidence',
  );
  const check = {
    ...base,
    id: 'depth:2',
  ...
  assert.equal(report.totals.derived_state, 1);
  assert.deepEqual(report.requirements[2].levels, []);
  for (const invalid of [[], ['missing']])
    assert.throws(
      () => aggregateEvidence(ids, [call, { ...check, source_call_ids: invalid }]),
      /successful native source calls/,
    );
  assert.throws(
    () => aggregateEvidence(ids, [{ ...call, status: 'skipped' }, check]),
    /successful native source calls/,
  );
...
test('report grouping preserves every original requirement and never transfers shared-schema evidence', () => {
  const ids = [
    'adjustment:exposure',
    'adjustment:masks[].adjustments.exposure',
    'adjustment:masks[].subMasks[]<flow>.opacity',
    'adjustment:aiPatches[].subMasks[]<flow>.opacity',
    'parameter:export.quality',
    'parameter:batch_export.options.quality',
  ];
  const report = aggregateEvidence(ids, [
    {
      id: '1',
      type: 'call',
      evidence_format: 'rapidraw-native-evidence-v1',
      ...
      level: 'native_call',
      basis: 'direct_input',
      requirements: [ids[0], ids[2], ids[4]],
    },
  ]);
  assert.deepEqual(
  ...
  assert.equal(report.totals.required, ids.length);
  assert.equal(report.totals.untested, 3);
  assert.deepEqual(report.requirements[3].levels, []);
  assert.deepEqual(report.groups.find((g) => g.family === 'submask:flow').contexts, [
    'local mask component',
    'retouch patch component',
  ]);
...
test('evidence recursively redacts credentials and large encoded assets', () => {
  assert.deepEqual(
    sanitize({
      token: 'private',
      child: { apiKey: 'private', authorization: 'Bearer private', dataBase64: 'image' },
      path: 'fixture.png',
    }),
    {
      token: '[redacted]',
      child: { apiKey: '[redacted]', authorization: '[redacted]', dataBase64: '[omitted 5 characters]' },
      path: 'fixture.png',
    },
  );
});
test('independent PNG decoder preserves exact synthetic pixels and detects corrupt bytes', () => {
  const bytes = fixturePng(20, 12, (x, y) => [x * 10, y * 10, 100]);
  const decoded = decodePng(bytes);
  assert.equal(decoded.width, 20);
  assert.equal(decoded.height, 12);
  assert.equal(decoded.pixels[(3 * 20 + 2) * 3], 20 / 255);
  assert.equal(decoded.pixels[(3 * 20 + 2) * 3 + 1], 30 / 255);
  assert.equal(pixelDifference(decoded, decodePng(bytes)).maximum, 0);
  assert.ok(mean(decoded) > 0);
