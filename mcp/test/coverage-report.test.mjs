...
async function fixture(t, change = {}) {
  ...
  const records = change.records ?? [{ id: 'fixture:1', evidence_format: 'rapidraw-native-evidence-v1', execution: 'real_native_mcp', type: 'call', method: 'render', requirements, status: 'passed', level: 'native_call' }];
  const report = aggregateEvidence(requirements, records);
  const summary = { provenance, status: 'passed', calls: records.filter((r) => r.type === 'call').length, ...report.totals, ...change.summary };
...
const run = (directory) => execute(process.execPath, [script, directory], { env: { ...process.env, RAPIDRAW_COVERAGE_OUTPUT: join(directory, 'combined-coverage.json') } });
async function rejectedWithoutOutput(directory, pattern) {
  await assert.rejects(run(directory), (error) => { assert.match(error.stderr, pattern); return true; });
...
test('aggregator refuses untagged legacy assertion failure even when every ledger call passed', async (t) => {
  ...
  const directory = await fixture(t, { summary: { status: 'failed', failed: 0 }, coverage: { error: 'AssertionError: persisted pixels changed after reconnect' } });
...
test('aggregator requires a complete passed summary and rejects contradictory completion reports', async (t) => {
  for (const [name, change, pattern] of [
    ...
    ['stale completion provenance', { summary: { provenance: { runtime_tree_sha256: 'c'.repeat(64), binary_sha256: 'b'.repeat(64) } } }, /completion report differs/],
...
test('aggregator still combines completed matching runs and retains call evidence', async (t) => {
  ...
  await execute(process.execPath, [script, first, second, '--require-native-tools'], { env: { ...process.env, RAPIDRAW_COVERAGE_OUTPUT: join(first, 'combined-coverage.json') } });
  const result = JSON.parse(await readFile(join(first, 'combined-coverage.json'), 'utf8'));
  assert.equal(result.runs.length, 2); assert.equal(result.totals.failed, 0); assert.equal(result.totals.native_call, 1);
...
test('aggregator namespaces explicit derived source references and cannot treat them as direct inputs', async (t) => {
  ...
  const records = [
    { ...base, id: 'depth:1', type: 'call', method: 'generate_depth', level: 'native_call', basis: 'direct_input', requirements: [requirements[0]] },
    { ...base, id: 'depth:2', type: 'check', level: 'native_assertion', basis: 'derived_state', source_call_ids: ['depth:1'], requirements: [requirements[1]] },
  ];
  ...
  assert.equal(report.totals.direct_input, 1); assert.equal(report.totals.pixel_assertion, 0);
  const second = await fixture(t, { requirements, records });
  await assert.rejects(execute(process.execPath, [script, second, '--require-direct-adjustments'], { env: { ...process.env, RAPIDRAW_COVERAGE_OUTPUT: join(second, 'combined-coverage.json') } }), (error) => { assert.match(error.stderr, /derived state does not establish/); return true; });
...
test('strict native tool gate fails before writing an apparently successful aggregate', async (t) => {
  const directory = await fixture(t, { requirements: ['tool:render', 'tool:export'], records: [{ id: '1', evidence_format: 'rapidraw-native-evidence-v1', execution: 'real_native_mcp', type: 'call', method: 'render', status: 'passed', level: 'native_call', requirements: ['tool:render'] }] });
  await assert.rejects(execute(process.execPath, [script, directory, '--require-native-tools'], { env: { ...process.env, RAPIDRAW_COVERAGE_OUTPUT: join(directory, 'combined-coverage.json') } }), (error) => { assert.match(error.stderr, /without successful real native calls/); return true; });
  await assert.rejects(readFile(join(directory, 'combined-coverage.json')), { code: 'ENOENT' });
});
