...
function create(options = {}) {
  return new NativeBridge({
    binary,
    workspace: '/private/tmp/rapidraw-mcp-tests',
    timeoutMs: 2000,
    log: () => {},
    ...options,
  });
...
test('serializes native requests, frames split lines, and recovers from ordinary errors', async (t) => {
  ...
  const results = await Promise.all([
    bridge.request('get_session', { session_id: 'slow' }),
    bridge.request('get_session', { session_id: 'next' }),
  ]);
...
test('crash rejects all queued requests without respawning', async (t) => {
  ...
  const results = await Promise.allSettled([
    bridge.request('get_session', { session_id: 'crash' }),
    bridge.request('get_session', { session_id: 'must-not-run' }),
  ]);
...
test('missing binary gives actionable startup failure', async (t) => {
  const bridge = create({ binary: '/nonexistent/rapidraw' });
  t.after(() => bridge.close());
...
test('native startup error preserves actionable workspace code without unrelated stderr', async (t) => {
  const bridge = create({ env: { ...process.env, FAKE_START_ERROR: '1' } });
  t.after(() => bridge.close());
