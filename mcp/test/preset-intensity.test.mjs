...
const { applyPresetIntensity } = await import(`data:text/javascript;base64,${Buffer.from(compiled.outputText).toString('base64')}`);

const base = () => ({
  exposure: 0.8, temperature: -3, tint: 4, colorNoiseReduction: 22, sharpness: 17, contrast: 10,
  lutPath: null, lutIntensity: 100, lutIsSceneReferred: false,
  curves: { luma: [{ x: 0, y: 0 }, { x: 128, y: 136 }, { x: 255, y: 255 }] }, masks: [],
});
...
test('partial film presets preserve omitted base corrections and restore zero exactly', () => {
  const captured = base(), preset = film(), untouched = structuredClone(captured);
...
test('changing intensity leaves subsequent edits to omitted controls intact', () => {
  const captured = base(), preset = film();
...
test('curves interpolate at common coordinates and noncurve arrays keep their topology', () => {
  const captured = base();
  const preset = { curves: { luma: [{ x: 0, y: 6 }, { x: 64, y: 72 }, { x: 255, y: 249 }] }, masks: [{ id: 'selected-mask' }] };
  const half = applyPresetIntensity(preset, 50, captured);
  assert.deepEqual(half.curves.luma.map((point) => point.x), [0, 64, 128, 255]);
