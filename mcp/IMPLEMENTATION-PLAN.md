...
Freeze the implementation before inspecting a fresh photographic holdout. Previously inspected images remain regression inputs. Validate native Windows/Linux behavior and fresh-machine installation on those actual environments before claiming support from a local macOS result. The [gap assessment](GAP-ASSESSMENT.md) records priorities and remaining acceptance boundaries.
