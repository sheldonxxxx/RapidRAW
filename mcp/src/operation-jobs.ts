...
interface OperationJob {
  ...
  stage: string; attempt: number; created_at: string; updated_at: string;
  bundle?: string; bundle_sha256?: string; sources?: { path: string; sha256: string }[];
  settings_sha256?: string;
  models?: CapturedModels;
  result?: JsonObject; error?: string;
}
...
const fail = (code: string, message: string): never => { throw new BridgeError(code, message); };
const jobId = (value: unknown): string => {
  if (typeof value !== 'string' || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(value)) fail('INVALID_ARGUMENT', 'Expected a job UUID');
...
const inside = (root: string, path: string): boolean => {
  ...
  return child !== '' && child !== '..' && !child.startsWith(`..${sep}`) && !isAbsolute(child) && resolve(path) === path;
};
const confined = async (root: string, path: unknown, kind: 'file' | 'directory'): Promise<string> => {
  if (typeof path !== 'string' || !inside(root, path)) fail('INVALID_PATH', 'Captured asset must belong to its managed directory');
  const target = path as string;
  if ((await lstat(root)).isSymbolicLink() || await realpath(root) !== root) fail('INVALID_PATH', 'Managed asset root cannot be a symlink');
  let current = root;
  ...
  if (kind === 'file' ? !metadata.isFile() : !metadata.isDirectory()) fail('INVALID_PATH', `Expected a regular ${kind}`);
...
export class OperationJobs {
  ...
  private async init(): Promise<void> {
    this.initialized ??= (async () => {
      ...
      if ((await lstat(this.root)).isSymbolicLink() || await realpath(this.root) !== this.root) fail('INVALID_PATH', 'Operation jobs require a real directory inside the workspace');
      for (const entry of await readdir(this.root, { withFileTypes: true })) {
        if (!entry.isDirectory() || !/^[0-9a-f-]{36}$/.test(entry.name)) continue;
        const job = await this.read(entry.name).catch((error) => { if (!(error instanceof BridgeError && error.code === 'JOB_NOT_FOUND')) this.options.log?.(`Operation job ${entry.name} requires repair: ${this.diagnostic(error).message}`); return undefined; });
        if (!job) continue;
        if (job.status === 'running') { job.status = 'interrupted'; job.stage = 'Explicit resume required; captured work will restart'; await this.save(job); }
      }
    })();
  ...
  private async read(id: string): Promise<OperationJob> {
    ...
    try {
      ...
      manifestFound = true;
      if (job.job_id !== id || !methods.has(job.operation) || !['running','succeeded','failed','cancelled','interrupted'].includes(job.status) || !Number.isSafeInteger(job.attempt) || job.attempt < 1 || job.attempt > 1000 || !job.arguments || typeof job.arguments !== 'object' || Array.isArray(job.arguments)) fail('INVALID_JOB', 'Invalid job identity, operation, attempt, state or arguments');
      if ('token' in job.arguments || job.arguments.mode === 'generative') fail('INVALID_JOB', 'Job cannot contain provider credentials or remote generation');
      if (job.bundle !== undefined) {
        if (!/^[a-f0-9]{64}$/.test(job.bundle_sha256 ?? '')) fail('INVALID_JOB', 'Captured bundle must have a manifest SHA-256');
        await confined(join(this.workspace, 'bundles'), job.bundle, 'directory');
      }
      if (job.sources !== undefined) {
        if (!Array.isArray(job.sources) || job.sources.length < 2 || job.sources.length > 100) fail('INVALID_JOB', 'Invalid captured merge sources');
      ...
      if (job.operation === 'merge' ? !job.sources || !!job.bundle : !job.bundle || !!job.sources) fail('INVALID_JOB', 'Captured inputs do not match operation');
      if (job.settings_sha256 !== undefined && !/^[a-f0-9]{64}$/.test(job.settings_sha256)) fail('INVALID_JOB', 'Invalid captured settings hash');
      if (job.models !== undefined) {
        const requiredKind = modelKind(job.operation, job.arguments);
        if (!requiredKind || !job.models || job.models.kind !== requiredKind || !Array.isArray(job.models.assets) || !job.models.assets.length || job.models.assets.length > 32) fail('INVALID_JOB', 'Invalid captured model group');
        const names = new Set<string>();
        for (const asset of job.models.assets) {
          if (!asset || !modelName(asset.name) || names.has(asset.name) || !/^[a-f0-9]{64}$/.test(asset.sha256) || asset.path !== join(directory, 'models', asset.name)) fail('INVALID_JOB', 'Invalid captured model identity, path or hash');
          names.add(asset.name); await confined(join(directory, 'models'), asset.path, 'file');
        }
      }
      return job;
    } catch (error) {
  ...
  private diagnostic(error: unknown): { code: string; message: string } {
    return { code: error instanceof BridgeError ? error.code : 'INVALID_JOB', message: error instanceof Error ? error.message : 'Invalid operation job' };
  }
  private async save(job: OperationJob): Promise<void> {
    ...
    let file;
    try {
      file = await open(temp, 'wx'); await file.writeFile(JSON.stringify(job, null, 2)); await file.sync(); await file.close(); file = undefined;
  ...
  dispatch(method: string, params: JsonObject): Promise<JsonObject> {
    const task = this.serial.then(async () => {
      ...
      switch (method) {
        case 'start_operation': return this.start(params);
        case 'get_operation_job': return this.info(await this.read(jobId(params.job_id)));
        case 'list_operation_jobs': {
          const jobs = [], invalid_jobs = [];
          for (const entry of await readdir(this.root, { withFileTypes: true })) if (entry.isDirectory() && /^[0-9a-f-]{36}$/.test(entry.name)) {
            try { jobs.push(this.info(await this.read(entry.name))); }
            catch (error) { if (!(error instanceof BridgeError && error.code === 'JOB_NOT_FOUND')) invalid_jobs.push({ job_id: entry.name, error: this.diagnostic(error), recoverable: false }); }
          }
        ...
        case 'resume_operation_job': {
          ...
          if (!['interrupted', 'failed', 'cancelled'].includes(job.status)) fail('JOB_NOT_RESUMABLE', 'Only interrupted, failed or cancelled work can restart');
          if (job.attempt >= 1000) fail('JOB_NOT_RESUMABLE', 'Operation job attempt limit reached');
          await this.verifyModels(job);
          job.attempt++; job.status = 'running'; job.stage = 'Starting captured operation'; delete job.error; delete job.result;
          await this.save(job); this.launch(job); return this.info(job);
        }
        default: return fail('UNKNOWN_METHOD', method);
      }
    });
  ...
  private async start(params: JsonObject): Promise<JsonObject> {
    ...
    const id = randomUUID(); const directory = join(this.root, id);
    await mkdir(directory);
    const job: OperationJob = { job_id: id, operation, arguments: args, status: 'running', stage: 'Captured input', attempt: 1, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
    try {
    ...
    const settingsInfo = await lstat(settings).catch((error) => { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined; throw error; });
    if (settingsInfo) {
      if (!settingsInfo.isFile() || settingsInfo.isSymbolicLink()) fail('INVALID_PATH','Engine settings must be a regular file');
      const target = join(directory,'engine-settings.json'); await copyFile(settings,target); job.settings_sha256 = await hashFile(target);
    }
    if (operation === 'merge') {
      if (!Array.isArray(args.paths) || args.paths.length < 2 || args.paths.length > 100 || !['hdr', 'focus', 'panorama'].includes(String(args.kind))) fail('INVALID_ARGUMENT', 'Merge needs a valid kind and 2–100 absolute source paths');
      await mkdir(join(directory, 'sources'));
      job.sources = [];
      const paths = args.paths as string[];
      for (let i = 0; i < paths.length; i++) {
        const source = paths[i]!;
        if (typeof source !== 'string' || !isAbsolute(source) || !(await lstat(source)).isFile()) fail('INVALID_PATH', 'Merge inputs must be absolute regular files');
        const target = join(directory, 'sources', `${i}-${basename(source)}`);
        const before = await hashFile(source); await copyFile(source, target);
        if (await hashFile(target) !== before || await hashFile(source) !== before) fail('SOURCE_CHANGED', 'Source changed during capture');
      ...
      const bundle = await this.bridge.request('export_session_bundle', { session_id: args.session_id, ...(args.expected_revision === undefined ? {} : { expected_revision: args.expected_revision }), name: `job-${id}` });
      job.bundle = String(bundle.path ?? bundle.bundle_path);
      job.bundle_sha256 = String(bundle.manifest_sha256 ?? '');
      if (!/^[a-f0-9]{64}$/.test(job.bundle_sha256)) fail('INVALID_BUNDLE', 'Native bundle export returned no valid manifest SHA-256');
      ...
      if (operation === 'export') {
        if (typeof args.path !== 'string' || !basename(args.path)) fail('INVALID_ARGUMENT', 'Export needs a filename');
        args.path = basename(args.path as string); args.overwrite = false;
      }
    }
    await this.save(job); this.launch(job); return this.info(job);
  ...
  private async captureModels(directory: string, kind: ModelKind): Promise<CapturedModels | undefined> {
    ...
    const names = new Set<string>();
    const assets: { name: string; path: string; sha256: string; present: boolean; verified: boolean; installed: boolean }[] = [];
    for (const raw of group!.assets as unknown[]) {
      const asset = raw as Record<string, unknown>;
      if (!asset || !modelName(asset.name) || names.has(asset.name) || typeof asset.expected_sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(asset.expected_sha256) || asset.path !== join(modelRoot, asset.name) || typeof asset.present !== 'boolean' || typeof asset.verified !== 'boolean' || typeof asset.installed_app_copy_available !== 'boolean') fail('INVALID_MODEL_STATUS', 'Native model asset must have a safe name, confined path and expected SHA-256');
      const name = asset.name as string;
      names.add(name);
      const metadata = await lstat(asset.path as string).catch((error) => { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined; throw error; });
      if (metadata && (!metadata.isFile() || metadata.isSymbolicLink())) fail('INVALID_PATH', `Workspace model ${asset.name} must be a regular file`);
      assets.push({ name, path: asset.path as string, sha256: asset.expected_sha256 as string, present: !!metadata, verified: asset.verified === true && asset.present === !!metadata, installed: asset.installed_app_copy_available === true });
    }
    ...
    if (group!.ready !== true || assets.some((asset) => !asset.present || !asset.verified)) fail('MODEL_NOT_INSTALLED', `Required ${kind} model set is missing, partial or corrupt. Call install_model with kind='${kind}' in the parent workspace before starting this operation.`);
    ...
    for (const asset of assets) {
      ...
      if (await hashFile(source) !== asset.sha256) fail('MODEL_CHANGED', `Workspace model ${asset.name} differs from its expected SHA-256; run install_model with kind='${kind}' before retrying.`);
      const target = join(directory, 'models', asset.name); await copyFile(source, target);
      if (await hashFile(target) !== asset.sha256 || await hashFile(source) !== asset.sha256) fail('MODEL_CHANGED', `Workspace model ${asset.name} changed during capture; run install_model with kind='${kind}' before retrying.`);
  ...
  private async verifyModels(job: OperationJob): Promise<void> {
    ...
    for (const asset of job.models?.assets ?? []) {
      ...
      if (await hashFile(asset.path) !== asset.sha256) fail('MODEL_CHANGED', `Captured model ${asset.name} has a SHA-256 mismatch. Start a new operation after repairing parent models with install_model; the captured job is preserved.`);
    }
  }
  private launch(job: OperationJob): void {
    ...
    const completion = this.run(job, worker).catch((error: unknown) => { this.options.log?.(`Operation job ${job.job_id} persistence failed: ${error instanceof Error ? error.message : String(error)}`); }).finally(() => { if (this.active?.id === job.job_id) this.active = undefined; });
    ...
    try {
      ...
      if (job.models) {
        ...
        for (const asset of job.models.assets) {
          ...
          try { for await (const chunk of createReadStream(asset.path)) await file.writeFile(chunk); await file.sync(); }
          finally { await file.close(); }
          if (await hashFile(target) !== asset.sha256) fail('MODEL_CHANGED', `Captured model ${asset.name} changed while preparing the worker`);
        }
      }
      if (job.settings_sha256) {
        ...
        if (await hashFile(settings)!==job.settings_sha256) fail('SETTINGS_CHANGED','Captured engine settings hash mismatch');
        const workspace=join(this.root,job.job_id,`attempt-${job.attempt}`); await mkdir(workspace,{recursive:true}); await copyFile(settings,join(workspace,'engine-settings.json'));
      }
      const args = structuredClone(job.arguments);
      for (const source of job.sources ?? []) if (await hashFile(source.path) !== source.sha256) fail('SOURCE_CHANGED', 'Captured merge input hash mismatch');
  ...
  async close(): Promise<void> {
    this.closing = true;
    if (this.active) { await this.active.worker.close(); await this.active.completion; }
  }
}
