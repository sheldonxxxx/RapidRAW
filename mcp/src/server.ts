...
interface ResponseContext { method?: string; params?: JsonObject; readOnly?: boolean; requestId?: string | number; }
const responseBytes = (result: unknown, requestId: string | number = 0): number => Buffer.byteLength(JSON.stringify({ jsonrpc: '2.0', id: requestId, result }), 'utf8') + 1;
const textResult = (output: JsonObject): CallToolResult => ({ isError: true, content: [{ type: 'text', text: JSON.stringify(output) }], structuredContent: output });

function recoveryMetadata(result: JsonObject): JsonObject {
  const output: JsonObject = {};
  for (const key of ['id', 'session_id', 'result_session_id', 'parent_session_id', 'revision', 'mask_id', 'patch_id', 'version_id', 'job_id', 'path', 'bundle_path', 'working_path', 'status', 'ok', 'failed', 'succeeded', 'total']) {
    const value = result[key];
    if (typeof value === 'string' && value.length <= 2048 || typeof value === 'number' && Number.isFinite(value) || typeof value === 'boolean') output[key] = value;
  }
  for (const key of ['mask_ids', 'submask_ids', 'patch_ids']) if (Array.isArray(result[key])) {
...
function oversizedResult(result: JsonObject, bytes: number, context: ResponseContext): CallToolResult {
  ...
  const recovery: JsonObject = {
    method: context.method?.slice(0, 128) ?? 'unknown', request_completed: true, mutation_may_have_completed: mutation, retry_mutation: false,
  ...
  for (const key of ['results', 'items']) if (Array.isArray(result[key])) {
    const entries = result[key] as unknown[];
    recovery[key] = entries.slice(0, 16).map((entry) => isObject(entry) ? { ...recoveryMetadata(entry), ...(isObject(entry.result) ? { result: recoveryMetadata(entry.result) } : {}) } : {});
    recovery[`${key}_count`] = entries.length;
    recovery[`${key}_truncated`] = entries.length > 16;
  }
  return textResult({ error: { code: 'RESPONSE_TOO_LARGE', message: `The completed response requires ${bytes} UTF-8 JSON-RPC bytes, exceeding the ${MCP_RESPONSE_BUDGET_BYTES}-byte MCP response budget. The connection remains usable; follow recovery guidance before retrying.`, response_bytes: bytes, max_response_bytes: MCP_RESPONSE_BUDGET_BYTES }, recovery });
}
...
function toolError(error: unknown): CallToolResult {
  ...
  return textResult({ error: { code, message: message.slice(0, 4096), ...(message.length > 4096 ? { message_truncated: true } : {}) } });
}
...
function jsonResource(uri: URL, data: JsonObject, requestId: string | number) {
  ...
  if (bytes > MCP_RESPONSE_BUDGET_BYTES) throw new BridgeError('RESPONSE_TOO_LARGE', `RESPONSE_TOO_LARGE: Resource requires ${bytes} bytes; MCP budget is ${MCP_RESPONSE_BUDGET_BYTES}. Read session metadata using get_session(include_adjustments:false) and bounded preview regions. No mutation was requested; the connection remains usable.`);
  return response;
}
...
export function createServer(bridge: NativeBridge, jobs?: OperationJobs): McpServer {
  ...
  const capabilities = async (): Promise<JsonObject> => {
    ...
    const limits = { max_response_bytes: MCP_RESPONSE_BUDGET_BYTES, oversized_response: 'RESPONSE_TOO_LARGE', automatic_image_resizing: false };
    return jobs ? { ...result, methods: [...result.methods, ...OperationJobs.methods], background_operations: { isolation: 'separate native worker and captured bundle', progress: 'stage', cancellation: 'worker process only' }, transport_limits: limits } : { ...result, transport_limits: limits };
  };
  for (const definition of toolDefinitions) {
    server.registerTool(`rapidraw_${definition.method}`, {
      ...
      const respond = (result: JsonObject) => toolResult(result, { method: definition.method, params, readOnly: definition.readOnly, requestId: context.mcpReq.id });
      try {
        const available = await capabilities();
        if (definition.method === 'capabilities') return respond(available);
        if (definition.host) {
          if (!jobs) throw new BridgeError('UNSUPPORTED_METHOD', 'Operation workers require the configured server entry point.');
  ...
  server.registerPrompt('pro_photo_edit', {
    ...
    argsSchema: z.object({ path: z.string().min(1).max(16384).describe('Absolute source photo path.'), intent: z.string().max(65536).optional().describe('Desired treatment and delivery format; default natural professional edit.') }).strict(),
