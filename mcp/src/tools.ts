...
const generationOptions = z.object({
  ...
  profile: z.string().min(1).max(64).regex(/^[A-Za-z0-9][A-Za-z0-9_.-]*$/).optional(),
  megapixels: z.number().min(0.0625).max(16).optional(),
}).strict().describe('Optional reproducible settings for generative mode with an AI Connector. Explicit options require protocol-v2 capabilities and an advertised profile/resolution. Saved with the patch; omit to retain connector defaults.');
...
const exportOptions = {
  ...
  color_profile: z.enum(['auto', 'srgb', 'none']).optional().describe('auto embeds an sRGB ICC profile in JPEG/PNG/TIFF/WebP; srgb requires support; none omits explicit embedding.'),
...
const enhancementRequest = z.object({
  ...
  region: z.tuple([z.number().int().nonnegative(), z.number().int().nonnegative(), z.number().int().positive(), z.number().int().positive()]).optional().describe('[x,y,width,height]. Masks use the oriented canvas before crop. Restoration uses rendered pixels after crop and returns only this region when supplied.'),
}).strict();
...
export const toolDefinitions: ToolDefinition[] = [
  tool('enhancement_models', 'Inspect local matting, scene/face parsing, deblur and 2x reconstruction models, installation status and runtime policy.', {}, true),
  tool('install_enhancement_model', 'Install a verified local enhancement model. Matting and face models can download; landscape/deblur/upscale use the repository exporter and require its ONNX path and SHA-256. No photographs are uploaded.', { model_id: z.enum(['matting', 'face', 'landscape', 'deblur', 'upscale']), path: path.optional(), sha256: z.string().regex(/^[a-fA-F0-9]{64}$/).optional() }, false, { network: true, timeoutMs: 1_800_000 }),
  tool('enhance', 'Run native learned mask refinement, semantic selection, deblur or 2x reconstruction. Prefer start_operation for cancellable work. Refinement replaces one mask component preserving siblings and grade. Restoration processes rendered sRGB into a separate session with edits baked; the parent remains editable. Fast/balanced matting limits inference resolution; quality processes native boundary tiles. Review actual detail, receipt timings and enhancement.warnings; partial refinement can retain coarse coverage.', { session_id: sessionId, expected_revision: z.number().int().nonnegative(), request: enhancementRequest, mask_id: z.string().min(1).optional(), sub_mask_id: z.string().min(1).optional(), name: z.string().min(1).max(200).optional() }, false, { timeoutMs: 1_800_000 }),
  tool('manage_presets', 'Manage workspace-owned presets without modifying the desktop library: list, save current edits, import, export or remove. On save, adjustment_keys selects only the intended controls; omit adjustment_keys for the existing full save behavior. Saved masks and geometry are explicit options; LUT dependencies are portable.', { action: z.enum(['list','save','import','export','remove']), session_id: sessionId.optional(), expected_revision: z.number().int().nonnegative().optional(), name: z.string().min(1).max(200).optional(), id: z.string().min(1).optional(), path: path.optional(), include_masks: z.boolean().optional(), include_geometry: z.boolean().optional(), adjustment_keys: z.array(z.string().min(1).max(100)).min(1).max(100).refine((keys) => new Set(keys).size === keys.length, 'Adjustment keys must be unique').optional().describe('Save only these exact top-level keys from the native adjustment schema. Masks/geometry require their include option. Select lutPath, lutIntensity and lutIsSceneReferred together for LUTs; include curves with any UI curve controls. Omit exposure, temperature, tint and detail keys to preserve per-photo corrections when applied.') }),
  tool('manage_luts', 'List, import, export or remove workspace-owned LUT assets. Imported files are parsed and copied; removal never deletes desktop or source files.', { action: z.enum(['list','import','export','remove']), name: z.string().min(1).max(200).optional(), id: z.string().min(1).optional(), path: path.optional() }),
  tool('fork_session', 'Create an independent candidate from current edits or a named version, preserving derived RAW interpretation and owned assets.', { ...mutation, version_id: z.uuid().optional(), label: z.string().trim().min(1).max(200).optional() }),
  tool('export_session_bundle', 'Save a portable, hash-verified directory bundle containing source, edits, versions and owned assets. Returns a path under workspace/bundles.', { ...mutation, name: z.string().min(1).max(120).optional() }),
  tool('import_session_bundle', 'Validate a portable bundle and import it as a new isolated session. Rejects unsafe paths, missing/corrupt assets and invalid state.', { path, expected_manifest_sha256: z.string().regex(/^[a-f0-9]{64}$/).optional() }),
  tool('diff_versions', 'Describe adjustment, mask, metadata and asset changes between named references; omit a side to use current edits. Does not mutate history.', { ...session, from_version_id: z.uuid().optional(), to_version_id: z.uuid().optional() }, true),
  tool('copy_adjustments', 'Copy explicitly selected adjustment keys to target sessions with per-target revision checks and outcomes. Geometry-dependent controls require equal source dimensions or explicit exclusion.', { ...mutation, version_id: z.uuid().optional(), keys: z.array(z.string().min(1)).min(1).max(100), geometry: z.enum(['exclude', 'require_same_dimensions']), targets: z.array(z.object({ session_id: sessionId, expected_revision: z.number().int().nonnegative() }).strict()).min(1).max(100), mode: z.enum(['merge', 'replace_selected']).optional() }),
  tool('map_coordinates', 'Map points, strokes and region boundaries between the loaded oriented source, pre-crop mask canvas, rendered image and a specified preview. Reports unmappable points; do not guess geometry from preview scale alone.', { ...session, from: z.enum(['oriented_source', 'mask', 'rendered', 'preview']), to: z.enum(['oriented_source', 'mask', 'rendered', 'preview']), points: z.array(mappedPoint).max(4096).optional(), strokes: z.array(z.array(mappedPoint).max(4096)).max(100).optional(), regions: z.array(region).max(100).optional(), preview: z.object({ width: z.number().int().positive(), height: z.number().int().positive(), region: renderRegion.optional() }).strict().optional() }, true),
  tool('preflight', 'Check source dimensions, native GPU limits, model availability and output compatibility before expensive processing. Estimates are advisory and do not claim photographic quality.', { ...session, operation: z.enum(['render', 'export', 'denoise', 'mask_generate', 'retouch', 'merge']).optional(), keep_metadata: z.boolean().optional(), format: exportOptions.format, bit_depth: exportOptions.bit_depth, model_kind: z.enum(['masks', 'inpaint', 'denoise']).optional() }, true),
  tool('sample_region', 'Measure robust rendered RGB and luminance in an exact region, labelled original or edited sRGB. Optional neutral-patch white-balance suggestion never applies an edit or claims sensor RAW measurements.', { ...session, region: renderRegion, stage: z.enum(['edited', 'original']).optional(), suggest_white_balance: z.boolean().optional() }, true),
  tool('start_operation', 'Capture an immutable input and start merge, export, negative conversion, AI masking/depth, enhancement or local retouch in an independent native worker. Returns job_id. Editing remains available; cancellation stops only this worker.', { operation: z.enum(['merge', 'export', 'negative_convert', 'mask_generate', 'generate_depth', 'retouch', 'enhance']), arguments: record }, false, { host: true }),
  tool('get_operation_job', 'Inspect a background operation status, stage, error and durable result. No automatic replay after interruption.', job, true, { host: true }),
  tool('list_operation_jobs', 'List durable background operation records including interrupted jobs after reconnect.', {}, true, { host: true }),
  tool('cancel_operation_job', 'Cancel a background operation by terminating its isolated worker; preserve captured inputs and keep the main editing engine running.', job, false, { host: true, idempotent: true }),
  tool('resume_operation_job', 'Explicitly restart failed, cancelled or interrupted captured work in a fresh worker workspace. Does not resume partial native computation.', job, false, { host: true }),
  tool(
    'inspect_adjustments',
  ...
  tool('mask_update', 'Patch an existing mask or edit individual submasks atomically. Native validation rejects malformed masks; use current revision.', { ...mutation, mask_id: z.string().min(1), patch: record.optional(), submask_operations: z.array(record).min(1).max(100).optional() }),
  ...
  tool('mask_generate', 'Generate a local AI mask. Subject include/exclude points use the full mask canvas before crop. New point-guided masks require a region or positive point. refine replaces only an existing AI-subject submask, preserving IDs, siblings and grade; requires expected_revision. Inspect returned refinement.prior_mode and review the mask before edits.', { ...mutation, kind: z.enum(['subject', 'foreground', 'sky', 'depth']), name: z.string().max(200).optional(), region: region.optional(), include_points: z.array(point).max(64).optional().describe('Subject points to include; at most 64 include/exclude points combined.'), exclude_points: z.array(point).max(64).optional().describe('Subject points to exclude; exclude-only refinement requires an existing prior mask.'), refine: z.object({ mask_id: z.string().min(1).max(128), sub_mask_id: z.string().min(1).max(128).optional() }).strict().optional().describe('Update the named ai-subject submask; omit sub_mask_id only when exactly one subject submask exists.'), parameters: record.optional(), adjustments: adjustments.optional() }),
  ...
  tool('merge', 'Merge multiple source exposures into HDR, focus stack or panorama in a new isolated session. Original images are preserved. Panorama requires every supplied image to connect through reliable matches; disconnected inputs fail instead of producing a partial result.', { kind: z.enum(['hdr', 'focus', 'panorama']), paths: z.array(path).min(2).max(100) }, false, { timeoutMs: 1_800_000 }),
