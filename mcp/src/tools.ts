...
export const toolDefinitions: ToolDefinition[] = [
  tool('manage_presets', 'Manage workspace-owned presets without modifying the desktop library: list, save current edits, import, export or remove. Saved masks and geometry are explicit options; LUT dependencies are portable.', { action: z.enum(['list','save','import','export','remove']), session_id: sessionId.optional(), expected_revision: z.number().int().nonnegative().optional(), name: z.string().min(1).max(200).optional(), id: z.string().min(1).optional(), path: path.optional(), include_masks: z.boolean().optional(), include_geometry: z.boolean().optional() }),
