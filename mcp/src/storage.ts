...
export async function requireFreeSpace(path: string, minimumGiB = 20): Promise<void> {
  ...
  if (available < BigInt(Math.ceil(minimumGiB * 1024 ** 3))) {
    throw new Error(
      `STORAGE_LOW: ${(Number(available) / 1024 ** 3).toFixed(1)} GiB free; ${minimumGiB} GiB required. Free rebuildable caches or archive completed work before starting another run.`,
    );
  }
}
