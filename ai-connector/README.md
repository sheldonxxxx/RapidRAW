...
[profiles/profiles.json](profiles/profiles.json) advertises eight examples: Klein 4B, Klein 4B with closer context, Klein 9B KV, Boogu Edit Turbo, an experimental Klein 4B native edit, Qwen Image 2.1, Qwen Image 2.1 Remove and Qwen remove PE. Remove entries whose models are unavailable and keep `default_profile` set to an enabled entry. Restart the connector after changing the catalog or a configuration file.
...
The `qwen21-v1` profile appears as **Qwen Image 2.1** in the workflow selector and supports 1 MP and 2 MP context generation. It uses 30 steps, CFG 1, Euler/simple, the local context as its first image, up to four additional reference images, and the native Qwen latent output. With attached references, the connector adds image-role instructions asking Qwen to edit the source scene and use uploads only for the requested appearance or properties. The private receipt retains both the original prompt and `effective_prompt`. This reduces ambiguity; it does not guarantee placement, pose, or scene preservation. Supply ordered references through `reference_images_base64` in the `/inpaint` request (an array of up to four base64-encoded images). Each reference must be PNG, JPEG or WebP, at most 9 MiB and 16 megapixels. The connector rounds generation dimensions to multiples of 32. RapidRAW composites the result through the removal mask. The 30-step value is a starting default; it does not guarantee preserved structure inside the selected area or prevent broad tonal bands in skies. Inspect the repair at native size.

For a quick removal, select **Qwen Image 2.1 · Remove** and brush the object. This separate workflow supplies a general removal instruction itself, so no text prompt is needed; the ordinary Qwen workflow sends the entered prompt unchanged when no reference images are attached. The connector records the instruction in its private receipt and returns only the selected-area patch. The mask limits the final composite but is not a native Qwen inpainting mask, so an unwanted object can still be regenerated. Inspect the subject and repair boundary at native size; use a scene-specific edit prompt or a different repair when it fails.

For prompt-enhanced removal, select **Qwen remove PE**, brush the unwanted content and enter a short removal instruction such as “remove the distant birds.” The connector sends the context crop and an aligned black-and-white guide to the Qwen Image 2.1 PE-I2I encoder. It asks PE to describe the edit without mentioning the guide. PE's final `rewritten_prompt` goes unchanged to Qwen Image 2.1, which receives only the photo crop as its image input. The guide is not sent to Qwen. The original selection still controls RapidRAW's final composite. This 1 MP profile uses 16 Euler/simple steps and does not accept extra reference images. The private evidence keeps the original and rewritten prompts, both Comfy IDs and both workflows; the connector receipt does not save PE's intermediate reasoning. A PE error stops generation instead of falling back to a different prompt.
...
**Qwen remove PE** uses `qwen3vl_8b_w4a8.safetensors` instead of the ordinary Qwen profile's INT8 encoder. It also needs `qwen3.5_9b_qwen_image_2.1_pe_i2i.int8_convrot.safetensors` in `models/text_encoders`, plus ComfyUI's `TextGenerate`, `ImageBatch` and `PreviewAny` nodes. Its model filenames are in the [profile configuration](profiles/configs/qwen21-remove-pe-v1.json). Download these weights under their upstream terms before enabling the profile.
