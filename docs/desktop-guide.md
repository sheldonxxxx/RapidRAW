...
In **Inpaint studio**, draw a selection, describe the repair, and choose **1–4 variations**. Each variation uses the same starting photo and selection; results are not applied automatically. Basic inpaint produces one result.

Click a thumbnail in **Photo generation history** to apply it; click the applied result again to disable it. History shows only results for the selected edit and is saved in the photo sidecar across restarts. **Reuse settings** copies a result's prompt and generation options to the current selection. **Delete result** removes an unwanted candidate without removing an already applied repair. Retained candidates increase sidecar size.

**Stop after current** saves the running result and skips the rest of the batch. Completed results remain available if a later variation fails. A fixed seed increments for each variation; leave it blank for new random seeds. Applying a historical result restores its saved selection; return to its original crop, orientation and geometry first if those have changed.

**Reference images** is a separate collapsible section below Variations. Expand it to add up to four images, select several files at once, or remove an image individually. Each image can be PNG, JPEG or WebP, up to 9 MiB and 16 megapixels. Choose **Qwen Image 2.1** or a supporting **Klein edit** workflow, then describe how each reference should guide the repair. The source photo remains the first workflow image; your references follow in thumbnail order. For Qwen, the connector adds instructions distinguishing the source scene from auxiliary references while retaining your original prompt in the receipt. Describe the intended pose and placement explicitly, and review the generated result before applying it. Uploads are disabled for unsupported workflows, including removal-only workflows and older connectors. References are sent to the configured connector, retained with the generation settings, and reduced to at most 1536 pixels on their longest edge for inference; the original photo is not resized.
