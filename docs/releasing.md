...
4. Run **Release: Build & Package App** from GitHub Actions on `main`, entering the existing tag. The workflow checks tag/version consistency and requires a draft release. It builds from the tag and uploads packages to that draft. Every package includes `mcp`; the matrix covers macOS ARM64/x86_64 and Linux x86_64. Android packaging is not part of this matrix.
