...
Optional [`start_denoise` with `method:"nonlocal"`](denoise/README.md#native-raw-denoise-through-mcp) instead writes a Bayer DNG before demosaicing. The GPU worker requires its own Python/CUDA configuration. The new session retains captured edits, and the DNG can also be opened independently through the normal RAW pipeline. Independent opening does not copy the session's adjustments unless its saved sidecar accompanies the file.
