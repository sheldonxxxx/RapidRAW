...
NIND/BM3D RAW denoising keeps a linear float TIFF and the source interpretation in the saved MCP session. Continue through that session for matching results; an independent GUI open of this intermediate TIFF does not reproduce the RAW interpretation from `.rrdata` alone. Use MCP export to create a portable display image. Original RAW working copies and their ordinary adjustment sidecars can be continued in the GUI.

Optional [`start_denoise` with `method:"nonlocal"`](denoise/README.md#native-raw-denoise-through-mcp) instead writes a Bayer DNG before demosaicing. The native ONNX runtime requires its own bundle/provider configuration. The new session retains captured edits, and the DNG can also be opened independently through the normal RAW pipeline. Independent opening does not copy the session's adjustments unless its saved sidecar accompanies the file.
